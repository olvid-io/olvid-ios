...
fileprivate let debugPrintPrefix = "[\(errorDomain)][OutboxMessageToOutboxMessageMigrationPolicyV20ToV21]"
fileprivate let entityName = "OutboxMessage"

final class OutboxMessageToOutboxMessageMigrationPolicyV20ToV21: NSEntityMigrationPolicy {
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        // Get the messageIdFromServer (raw UID) from the source instance and use it to set the rawMessageUidFromServer (Data) of the destination instance

        guard let messageIdFromServer = sInstance.value(forKey: "messageIdFromServer") as? UID? else {
            throw makeError(message: "Could not get the messageIdFromServer value")
        }
        
        dInstance.setValue(messageIdFromServer?.raw, forKey: "rawMessageUidFromServer")
        ...
        guard let messageId = sInstance.value(forKey: "messageId") as? UID else {
            throw makeError(message: "Could not get the messageId value")
        }
        
        dInstance.setValue(messageId.raw, forKey: "rawMessageIdUid")
