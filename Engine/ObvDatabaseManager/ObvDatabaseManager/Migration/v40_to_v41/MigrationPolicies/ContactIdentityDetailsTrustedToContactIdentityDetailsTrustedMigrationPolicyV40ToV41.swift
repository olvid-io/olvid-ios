...
final class ContactIdentityDetailsTrustedToContactIdentityDetailsTrustedMigrationPolicyV40ToV41: NSEntityMigrationPolicy, ObvErrorMaker {
    
    static let errorDomain = "ObvEngineMigrationV40ToV41"
    static let debugPrintPrefix = "[\(errorDomain)][ContactIdentityDetailsTrustedToContactIdentityDetailsTrustedMigrationPolicyV40ToV41]"
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        guard let optionalLabelAsString = sInstance.value(forKey: "photoServerLabel") as? String? else {
            throw Self.makeError(message: "Could not read the photoServerLabel attribute of a ContactIdentityDetailsTrusted entity")
        }
        ...
        if let label = optionalLabelAsString {
            if let uid = UID(hexString: label) {
                ...
                throw Self.makeError(message: "Could not turn the label attribute of a ContactIdentityDetailsTrusted entity into an UID")
            }
        }
