...
final class ContactGroupDetailsTrustedToContactGroupDetailsTrustedMigrationPolicyV40ToV41: NSEntityMigrationPolicy, ObvErrorMaker {
    
    static let errorDomain = "ObvEngineMigrationV40ToV41"
    static let debugPrintPrefix = "[\(errorDomain)][ContactGroupDetailsTrustedToContactGroupDetailsTrustedMigrationPolicyV40ToV41]"
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        let dInstance = try initializeDestinationInstance(forEntityName: "ContactGroupDetailsTrusted",
                                                          forSource: sInstance,
                                                          in: mapping,
                                                          manager: manager,
        ...
        guard let optionalLabelAsString = sInstance.value(forKey: "photoServerLabel") as? String? else {
            throw Self.makeError(message: "Could not read the photoServerLabel attribute of a ContactGroupDetailsTrusted entity")
        }
        ...
        if let label = optionalLabelAsString {
            if let uid = UID(hexString: label) {
                ...
                throw Self.makeError(message: "Could not turn the label attribute of a ContactGroupDetailsTrusted entity into an UID")
            }
        }
