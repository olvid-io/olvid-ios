...
extension GetSignedURLsSessionDelegate: URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        guard let (status, returnedValues) = RefreshInboxAttachmentSignedUrlServerMethod.parseObvServerResponse(responseData: dataReceived, using: log) else {
        ...
        case .ok:
            ...
            obvContext.performAndWait {
                ...
                do {
                    ...
                    self.errorForTracker = .coreDataFailure
                    return
                }
                ...
                if attachment.status == .cancelledByServer {
                    os_log("The attachment %{public}@ is cancelled by the server", log: log, type: .info, attachmentId.debugDescription)
                    self.errorForTracker = .attachmentWasCancelledByTheServer
                    return
                } else {
                    os_log("We successfully set new private URLs for the chunks of attachment %{public}@ (1)", log: log, type: .info, attachmentId.debugDescription)
                }
        ...
        case .deletedFromServer:
            // This occurs when we took too long to download an attachment (tipically more than a month). The attachment was deleted from the server, there is no chance to ever download it again.
            ...
            obvContext.performAndWait {
                ...
                assert(attachment.status == .cancelledByServer)
        ...
        case .generalError:
            os_log("Server reported general error during the DownloadPrivateURLsForAttachmentChunksCoordinator data task for attachment %{public}@", log: log, type: .fault, attachmentId.debugDescription)
