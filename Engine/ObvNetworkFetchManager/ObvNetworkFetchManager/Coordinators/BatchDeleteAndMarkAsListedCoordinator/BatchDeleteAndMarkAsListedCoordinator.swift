...
extension BatchDeleteAndMarkAsListedCoordinator {
    ...
    private func createBatchTask(ownedCryptoIdentity: ObvCryptoIdentity, fetchLimit: Int, delegateManager: ObvNetworkFetchDelegateManager, flowId: FlowIdentifier) -> Task<Void, Error> {
        return Task { [weak self] in
            ...
            let token = try await delegateManager.serverSessionDelegate.getValidServerSessionToken(for: ownedCryptoIdentity, currentInvalidToken: nil, flowId: flowId).serverSessionToken
            ...
            guard httpResponse.statusCode == 200 else {
                ...
                case 413:
                    os_log("🎉 [%@] Payload is too large (fetchLimit is %d)", log: Self.log, type: .debug, taskId, fetchLimit)
            ...
            case .invalidSession:
                _ = try await delegateManager.serverSessionDelegate.getValidServerSessionToken(for: ownedCryptoIdentity, currentInvalidToken: token, flowId: flowId)
                throw ObvError.tryAgainNowThatTheServerSessionIsValid
