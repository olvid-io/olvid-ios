...
public final class ObvServerGroupBlobLockServerMethod: ObvServerDataMethod {
        
    static let log = OSLog(subsystem: "io.olvid.server.interface.ObvServerGroupBlobLockServerMethod", category: "ObvServerInterface")

    public let pathComponent = "/groupBlobLock"
    ...
    public init(ownedIdentity: ObvCryptoIdentity, groupIdentifier: GroupV2.Identifier, lockNonce: Data, signature: Data, flowId: FlowIdentifier) {
    ...
    lazy public var dataToSend: Data? = {
        return [groupUID, lockNonce, signature].obvEncode().rawData
    }()
    ...
    public static func parseObvServerResponse(responseData: Data, using log: OSLog) -> Result<PossibleReturnStatus, Error> {
        ...
        case .invalidSignature:
            return .success(.invalidSignature)
        case .generalError:
