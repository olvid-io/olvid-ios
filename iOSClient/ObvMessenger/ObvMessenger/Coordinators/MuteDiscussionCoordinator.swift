...
final class MuteDiscussionCoordinator {

    fileprivate static let log = OSLog(subsystem: ObvMessengerConstants.logSubsystem, category: String(describing: MuteDiscussionCoordinator.self))

    private let internalQueue: OperationQueue = {
        let queue = OperationQueue()
        queue.maxConcurrentOperationCount = 1
        queue.qualityOfService = .userInitiated
        queue.name = "MuteDiscussionCoordinator internal queue"
        return queue
    }()
    ...
    private func observeNewMuteExpirationNotifications() {
        let log = MuteDiscussionCoordinator.log
        observationTokens.append(ObvMessengerInternalNotification.observeNewMuteExpiration(queue: internalQueue) { [weak self] (_) in
...
extension MuteDiscussionCoordinator: ScheduleNextTimerOperationDelegate {
