...
final class HardLinksToFylesCoordinator {
    ...
    fileprivate static let log = OSLog(subsystem: ObvMessengerConstants.logSubsystem, category: String(describing: HardLinksToFylesCoordinator.self))
    ...
    private let queueForNotifications = OperationQueue.createSerialQueue(name: "HardLinksToFylesCoordinator serial queue", qualityOfService: .default)
...
final class HardLinkToFyle: NSObject, QLPreviewItem {
    ...
    fileprivate init(fyleElement: FyleElement, currentSessionDirectoryForHardlinks: URL, log: OSLog) throws {
        let log = HardLinksToFylesCoordinator.log
