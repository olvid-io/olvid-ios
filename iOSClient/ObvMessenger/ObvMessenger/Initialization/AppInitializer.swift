...
final class AppInitializer {
    ...
    private let internalQueue: OperationQueue = {
        let queue = OperationQueue.createSerialQueue(name: "AppInitializer internal queue", qualityOfService: .userInteractive)
    ...
    /// This method servers as a completion handler of the `InitializeAppOperation`.
    private func initializeAppOperationDidFinish(result: Result<ObvEngine, InitializeAppOperationReasonForCancel>) {
        ...
        case .success(let obvEngine):
            ...
            op.queuePriority = .veryHigh
            internalQueue.addOperation(op)
            AppStateManager.shared.setStateToInitialized()
    ...
    func application(_ application: UIApplication, performActionFor shortcutItem: UIApplicationShortcutItem, completionHandler: @escaping (Bool) -> Void) {
        assert(Thread.isMainThread)
        guard AppStateManager.shared.currentState.isInitialized else { assertionFailure(); completionHandler(false); return }
        let log = self.log
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey: Any] = [:]) -> Bool {
        ...
        guard AppStateManager.shared.currentState.isInitialized else { assertionFailure(); return false }
        if url.scheme == "olvid" {
        ...
        internalQueue.addOperation { [weak self] in
            ...
            DispatchQueue(label: "Queue created for storing a completion handler").async {
                do {
                    ...
                    os_log("Could not store completion handler: %{public}@", log: log, type: .fault, error.localizedDescription)
                }
            }
        }
    }
