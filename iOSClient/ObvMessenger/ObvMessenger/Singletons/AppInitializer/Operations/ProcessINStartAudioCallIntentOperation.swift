...
final class ProcessINStartAudioCallIntentOperation: Operation {
    
    let startAudioCallIntent: INStartAudioCallIntent
    let obvEngine: ObvEngine
    
    init(startAudioCallIntent: INStartAudioCallIntent, obvEngine: ObvEngine) {
        self.startAudioCallIntent = startAudioCallIntent
        self.obvEngine = obvEngine
        super.init()
    }
    
    override func main() {
        guard let handle = startAudioCallIntent.contacts?.first?.personHandle?.value else { return cancel() }
        ...
        ObvStack.shared.performBackgroundTaskAndWait { (context) in
            ...
            if let callUUID = UUID(handle),
               let item = try? PersistedCallLogItem.get(callUUID: callUUID, within: context) {
                ...
                if let contact = PersistedObvContactIdentity.getAll(within: context)?.first(where: { $0.getGenericHandleValue(engine: obvEngine) == handle}) {
