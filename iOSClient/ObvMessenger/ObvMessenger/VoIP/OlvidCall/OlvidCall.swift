...
final class OlvidCall: ObservableObject {
    ...
    private static let log = OSLog(subsystem: ObvMessengerConstants.logSubsystem, category: "OlvidCall")
...
extension OlvidCall {
    ...
    func processAnswerCallJSONFromContact(_ contact: OlvidUserId, _ answerCallMessage: AnswerCallJSON) async throws -> OlvidCallParticipantInfo? {
        ...
        guard let participant = await getParticipant(remoteCryptoId: contact.remoteCryptoId) else { assertionFailure(); throw ObvError.couldNotFindParticipant }
...
extension OlvidCall {
    ...
    func processNewParticipantAnswerMessageJSONFromContact(_ contact: OlvidUserId, _ newParticipantAnswer: NewParticipantAnswerMessageJSON) async throws {
        guard let participant = await getParticipant(remoteCryptoId: contact.remoteCryptoId) else { assertionFailure(); return }
...
extension OlvidCall {
    ...
    private func getEndCallReport(reason: EndCallReason) -> CallReport? {
        ...
        case .callTimedOut:
            switch direction {
            case .incoming:
                return .missedIncomingCall(caller: callerOfIncomingCall?.info, participantCount: initialParticipantCount)
            case .outgoing:
...
extension OlvidCall {
    ...
    private func sendWebRTCMessage(to: OlvidCallParticipant, innerMessage: WebRTCInnerMessageJSON, forStartingCall: Bool) async throws {
        ...
        case .known(contactObjectID: let contactObjectID):
            await delegate.newWebRTCMessageToSend(webrtcMessage: message, contactID: contactObjectID, forStartingCall: forStartingCall)
...
extension OlvidCall: OlvidCallParticipantDelegate {
    ...
    func updateParticipants(with allCallParticipants: [ContactBytesAndNameJSON]) async throws {
        ...
        os_log("☎️ Entering updateParticipant(newCallParticipants: [ContactBytesAndNameJSON])", log: Self.log, type: .info)
    ...
    func receivedHangedUpMessage(from callParticipant: OlvidCallParticipant, messagePayload: String) async {
        ...
        case .known(let contactObjectID):
            do {
                ...
                fromOlvidUser = .known(contactObjectID: contact.typedObjectID, ownCryptoId: ownedCryptoId, remoteCryptoId: contactIdentifier.contactCryptoId, displayName: contact.customOrNormalDisplayName)
            } catch {
                assertionFailure()
                return
            }
...
extension OlvidCall {
    ...
    @MainActor
    func getParticipant(remoteCryptoId: ObvCryptoId) async -> OlvidCallParticipant? {
