...
struct OwnedIdentityTransferSummaryView: View {
    ...
    private var alertTitle: LocalizedStringKey {
        if let errorForAlert {
            return "COULD_NOT_PERFORM_OWNED_IDENTITY_TRANSFER_ALERT_\(ObvMessengerConstants.toEmailForSendingInitializationFailureErrorMessage)_\((errorForAlert as NSError).description)"
        } else {
            return "COULD_NOT_PERFORM_OWNED_IDENTITY_TRANSFER_ALERT_\(ObvMessengerConstants.toEmailForSendingInitializationFailureErrorMessage)"
        }
    }
    ...
    var body: some View {
        ...
        .alert(alertTitle, isPresented: $isAlertShown) {
            ...
            if let errorForAlert {
                Button("COPY_ERROR", role: .none) { UIPasteboard.general.string = (errorForAlert as NSError).description }
            }
        }
