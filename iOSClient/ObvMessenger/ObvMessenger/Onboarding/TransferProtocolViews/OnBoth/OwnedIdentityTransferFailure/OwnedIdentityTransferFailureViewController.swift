...
final class OwnedIdentityTransferFailureViewController: UIHostingController<OwnedIdentityTransferFailureView>, MFMailComposeViewControllerDelegate, OwnedIdentityTransferFailureViewActionsProtocol {
    ...
    func userWantsToSendErrorByEmail(errorMessage: String) async {
        ...
        composeVC.setToRecipients([ObvMessengerConstants.toEmailForSendingInitializationFailureErrorMessage])
    ...
    private struct Strings {
        static let mailSubject = NSLocalizedString("MAIL_SUBJECT_COULD_NOT_TRANSFER_PROFILE_ERROR", comment: "Mail subject")
        static let messageBody = { (errorMessage: String) in
            String.localizedStringWithFormat(NSLocalizedString("MAIL_BODY_COULD_NOT_TRANSFER_PROFILE_ERROR$@", comment: "mail body text"), errorMessage)
        }
    }
