                ...
                Group {
                    Section(footer: muteNotificationsFooter) {
                        Toggle(isOn: .init {
                            ...
                            Label("MUTE_NOTIFICATIONS", systemImage: ObvMessengerConstants.muteIcon.systemName)
                        }
                    }
                        ...
                        NotificationSoundPicker(selection: notificationSound.binding, showDefault: true) { sound -> Text in
                            ...
                            case .none:
                                if let globalNotificationSound = ObvMessengerSettings.Discussions.notificationSound {
                                    ...
                                    let systemSound = (try? AttributedString(markdown: "_\(CommonString.Title.systemSound)_")) ?? AttributedString(CommonString.Title.systemSound)
...
struct ObvLabelAlt: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline) {
            Label(title, systemImage: systemIcon.systemName)
            Spacer(minLength: 0)
        }
        .font(.body)
    }
