...
final class GroupCreationTypeHostingViewController: UIHostingController<GroupTypeView<PersistedObvContactIdentity>>, GroupTypeViewActionsProtocol {

    private weak var delegate: GroupCreationTypeHostingViewControllerDelegate?
    
    init(preselectedGroupType: GroupTypeValue?, selectedContacts: [PersistedObvContactIdentity], delegate: GroupCreationTypeHostingViewControllerDelegate) {
        self.delegate = delegate
        let actions = Actions()
        let view = GroupTypeView(model: .init(orderedContacts: selectedContacts, preselectedGroupType: preselectedGroupType), actions: actions)
