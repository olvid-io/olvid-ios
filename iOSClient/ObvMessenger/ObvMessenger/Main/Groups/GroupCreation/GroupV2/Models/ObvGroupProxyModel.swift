...
final class ObvGroupProxyModel {
    ...
    private(set) var admins = Set<PersistedObvContactIdentity>()
    ...
    init(ownedCryptoId: ObvCryptoId, editionType: NewGroupEditionFlowViewController.EditionType, directoryForTempFiles: URL) {
        ...
        case .modifyGroup(delegate: _, groupIdentifier: let groupIdentifier):
            ...
            self.selectedContacts = Set(group.otherMembers.compactMap { $0.contact })
            self.admins = Set(group.otherMembers.filter { $0.isAnAdmin }.compactMap { $0.contact })
    ...
    @MainActor
    func setselectedContacts(to newSelectedContacts: Set<PersistedObvContactIdentity>) {
        self.selectedContacts = newSelectedContacts
        self.admins = self.admins.intersection(newSelectedContacts)
    }
    ...
    @MainActor
    func changeContactAdminStatus(contactCryptoId: ObvTypes.ObvCryptoId, isAdmin: Bool) {
        guard let contact = selectedContacts.first(where: { $0.cryptoId == contactCryptoId }) else { assertionFailure(); return }
        if isAdmin {
            admins.insert(contact)
        } else {
            admins.remove(contact)
        }
    }
