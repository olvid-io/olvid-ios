...
protocol HorizontalContactsViewModelProtocol: ObservableObject {
    
    associatedtype ContactModel: SingleContactViewModelProtocol
    
    var canEditContacts: Bool { get }
...
/// Displays an horizontal list of selected contacts during a group creation.
struct HorizontalContactsView<Model: HorizontalContactsViewModelProtocol>: View {
    
    @ObservedObject var model: Model
    
    let actions: SingleContactViewActionsProtocol?
    ...
    var body: some View {
        ...
        ZStack {
            ...
            Text("SOME_OF_YOUR_CONTACTS_MAY_NOT_APPEAR_AS_GROUP_V2_CANDIDATES")
                .padding(16)
                .multilineTextAlignment(.center)
                .font(.callout)
                .foregroundStyle(.secondary)
            .opacity(model.orderedContacts.isEmpty ? 1.0 : 0.0)
            
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 20.0) {
                    ForEach(model.orderedContacts, id: \.cryptoId) { contact in
                        SingleContactView(model: contact, canEdit: model.canEditContacts, actions: actions)
                    }
                }
                .padding(.horizontal)
                .padding(.vertical, 8)
            }
            .opacity(model.orderedContacts.isEmpty ? 0.0 : 1.0)
...
extension PersistedObvContactIdentity: SingleContactViewModelProtocol {}

struct HorizontalContactsView_Previews: PreviewProvider {
    
    private final class Contacts: HorizontalContactsViewModelProtocol {
        typealias ContactModel = Contact
        
        var canEditContacts: Bool = true
        
        var orderedContacts: [Contact]
        
        func shouldDeleteContact(contact: Contact) {}
        
        init(orderedContacts: [Contact]) {
            self.orderedContacts = orderedContacts
        }
    }
    
    private final class Contact: SingleContactViewModelProtocol {
        ...
        static func == (lhs: HorizontalContactsView_Previews.Contact, rhs: HorizontalContactsView_Previews.Contact) -> Bool {
            lhs.cryptoId == rhs.cryptoId
        }

    }
    
    private final class Actions: SingleContactViewActionsProtocol {
