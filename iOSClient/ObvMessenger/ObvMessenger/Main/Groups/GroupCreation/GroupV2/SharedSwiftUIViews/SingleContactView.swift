...
protocol SingleContactViewModelProtocol: ObservableObject, Hashable, InitialCircleViewNewModelProtocol, SingleContactTextViewModelProtocol {
    var cryptoId: ObvCryptoId { get }
}


protocol SingleContactViewActionsProtocol {
    func userWantsToDeleteContact(cryptoId: ObvCryptoId) async
}

/// View shown during group creation. It is used in the horizontal scrolling list of selected user. It shows a circle with an initial, the name of the user, and, optionally, a button allowing to remove the user from the selection.
struct SingleContactView<Model: SingleContactViewModelProtocol>: View {
...
private struct DeleteButton<Model: SingleContactViewModelProtocol>: View {
    ...
    var body: some View {
        ...
        .onTapGesture {
            Task {
                await actions.userWantsToDeleteContact(cryptoId: model.cryptoId)
            }
        }
    }
...
struct SingleContactView_Previews: PreviewProvider {
    ...
    private final class Contact: SingleContactViewModelProtocol {
        ...
        static func == (lhs: SingleContactView_Previews.Contact, rhs: SingleContactView_Previews.Contact) -> Bool {
    ...
    static var previews: some View {
        Group {
            SingleContactView(model: Contact(cryptoId: cryptoId,
                                             firstName: "Jean-Baptiste", circledInitialsConfiguration: .contact(initial: "M",
                                                                                                                photo: nil, showGreenShield: false,
                                                                                                                showRedShield: false, cryptoId: cryptoId, tintAdjustementMode: .normal)),
                              canEdit: true,
                              actions: Actions())
