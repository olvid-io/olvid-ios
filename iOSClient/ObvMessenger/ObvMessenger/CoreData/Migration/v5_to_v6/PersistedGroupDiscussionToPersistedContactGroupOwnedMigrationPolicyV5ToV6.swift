...
fileprivate let debugPrintPrefix = "[\(errorDomain)][PersistedGroupDiscussionToPersistedContactGroupOwnedMigrationPolicyV5ToV6]"

final class PersistedGroupDiscussionToPersistedContactGroupOwnedMigrationPolicyV5ToV6: NSEntityMigrationPolicy {
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        // We only create the destination instance (an "owned" PersistedContactGroup) if the source (a PersistedGroupDiscussion) is owned. We check this first
        ...
        guard groupId.starts(with: ownedIdentity) else {
            debugPrint("\(debugPrintPrefix) The PersistedGroupDiscussion [\(title)] is not owned. We skip it.")
            return
        }
        
        // If we reach this point, the PersistedGroupDiscussion is one we own. So we must create a corresponding PersistedContactGroupOwned
        ...
        let newPersistedContactGroupOwned = PersistedContactGroupOwned(entity: description, insertInto: manager.destinationContext)
        ...
        try traversePropertyMappings { (propertyMapping, destinationName) in
            if let valueExpression = propertyMapping.valueExpression {
                ...
                newPersistedContactGroupOwned.setValue(destinationValue, forKey: destinationName)
            }
        }
        ...
        newPersistedContactGroupOwned.setValue(Set<PersistedPendingGroupMember>(), forKey: "pendingMembers")
        
        // We set the proper PersistedContactGroup.Category
        
        newPersistedContactGroupOwned.setValue(PersistedContactGroup.Category.owned.rawValue, forKey: "rawCategory")
        ...
        manager.associate(sourceInstance: sInstance, withDestinationInstance: newPersistedContactGroupOwned, for: mapping)
    ...
    override func createRelationships(forDestination dInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        // We get the PersistedGroupDiscussion in the destination context
        ...
        // We set the 'owner' relation to the ownedIdentity we retrieved
        
        dInstance.setValue(dPersistedOwnedIdentity, forKey: "owner")
