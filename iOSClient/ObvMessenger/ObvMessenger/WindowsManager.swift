...
final class WindowsManager {
    ...
    private var nonCallKitIncomingCallToShow: IncomingCall?
    ...
    private func observeNotifications() {
        observationTokens.append(ObvMessengerInternalNotification.observeShowCallViewControllerForAnsweringNonCallKitIncomingCall(queue: OperationQueue.main) { [weak self] (incomingCall) in
        ...
        observationTokens.append(ObvMessengerInternalNotification.observeAppStateChanged(queue: OperationQueue.main) { [weak self] (_, currentState) in
            guard let _self = self else { return }
            os_log("🪟 We received an AppStateChanged notification", log: _self.log, type: .info)
            let previousState = _self.currentState
            _self.currentState = currentState
            if let call = currentState.callInProgress {
                if previousState.callInProgress?.uuid != call.uuid || _self.callWindow.rootViewController == nil {
                    _self.callWindow.rootViewController = _self.makeCallViewController(call: call)
                }
            }
            os_log("🪟 We received an AppStateChanged notification, calling transitionToAppropriateWindow ", log: _self.log, type: .info)
...
extension WindowsManager {
    ...
    private func transitionToAppropriateWindow() {
         ...
         * In that case, we immediatly give the user an opportunity to answer/handup.
        ...
        case .initialized(iOSAppState: let iOSAppState, authenticated: let authenticated, authenticateAutomaticallyNextTime: let autoAuth, callInProgress: let callInProgress):
            ...
            case .notActive:
                ...
                if let (call, state) = callInProgress, !state.isFinalState {
                    if state == .initial && ObvMessengerSettings.VoIP.isCallKitEnabled && call is IncomingWebrtcCall {
            ...
            case .active:
                if let (call, state) = callInProgress, !state.isFinalState {
                    if state == .initial && ObvMessengerSettings.VoIP.isCallKitEnabled && call is IncomingWebrtcCall && ObvMessengerSettings.Privacy.lockScreen {
                        // Don't show call view since CallKit shows its own view.
                        return
                        ...
                        if let incomingCall = call as? IncomingCall {
                            guard incomingCall.userAnsweredIncomingCall || !ObvMessengerSettings.VoIP.isCallKitEnabled else { return } // Do nothing
                            assert(callWindow.rootViewController != nil)
                            transitionCurrentWindowTo(window: callWindow, animated: true)
                        } else {
                            assert(callWindow.rootViewController != nil)
    ...
    private func toggleCallView() {
        ...
        if !preferAppViewOverCallView, callWindow.rootViewController == nil, let call = currentState.callInProgress {
            callWindow.rootViewController = makeCallViewController(call: call)
        }
