...
extension NetworkSendFlowCoordinator: NetworkSendFlowDelegate {
    ...
    func deletedOutboxMessageWasCreated(messageId: ObvMessageIdentifier, flowId: FlowIdentifier) async {
        
        let logger = Logger(subsystem: ObvNetworkSendDelegateManager.defaultLogSubsystem, category: logCategory)

        guard let delegateManager = delegateManager else {
            logger.fault("The Delegate Manager is not set")
            assertionFailure()
            return
        }
        
        guard let notificationDelegate = delegateManager.notificationDelegate else {
            logger.fault("The notification delegate is not set")
            assertionFailure()
            return
        }

        ObvNetworkPostNotification.deletedOutboxMessageWasCreated(messageId: messageId, flowId: flowId)
            .postOnBackgroundQueue(queueForPostingNotifications, within: notificationDelegate)
