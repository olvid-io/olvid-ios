...
public enum ObvNetworkPostNotification {
	...
	case deletedOutboxMessageWasCreated(messageId: ObvMessageIdentifier, flowId: FlowIdentifier)
	...
	private enum Name {
		...
		case deletedOutboxMessageWasCreated
		...
		static func forInternalNotification(_ notification: ObvNetworkPostNotification) -> NSNotification.Name {
			...
			case .deletedOutboxMessageWasCreated: return Name.deletedOutboxMessageWasCreated.name
			}
		}
	}
	private var userInfo: [AnyHashable: Any]? {
		...
		case .deletedOutboxMessageWasCreated(messageId: let messageId, flowId: let flowId):
			info = [
				"messageId": messageId,
				"flowId": flowId,
			]
	...
	public static func observeDeletedOutboxMessageWasCreated(within notificationDelegate: ObvNotificationDelegate, queue: OperationQueue? = nil, block: @escaping (ObvMessageIdentifier, FlowIdentifier) -> Void) -> NSObjectProtocol {
		let name = Name.deletedOutboxMessageWasCreated.name
		return notificationDelegate.addObserver(forName: name, queue: queue) { (notification) in
			let messageId = notification.userInfo!["messageId"] as! ObvMessageIdentifier
			let flowId = notification.userInfo!["flowId"] as! FlowIdentifier
			block(messageId, flowId)
