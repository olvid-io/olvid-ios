...
public struct GroupV2 {
    ...
    public struct AdministratorsChain: ObvCodable, ObvErrorMaker {
        ...
        public struct Block {
            ...
            var allAdministratorIdentities: [ObvCryptoIdentity] {
            ...
            /// When signing a block, the signer always includes their identity first in the list of administrators. To compute the `signingAdministratorHint` we thus need to look for the first
            /// administrator of the signed block, make sure she is part of the administrators of the previous block, and use her identity first to check the signature. This should always work.
            fileprivate func signatureOnInnerDataIsValid(administratorsOfPreviousBlock administrators: [ObvCryptoIdentity], signingAdministratorHint: ObvCryptoIdentity) -> Bool {
                if administrators.map({ $0.getIdentity() }).contains(signingAdministratorHint.getIdentity()) {
                    if signatureOnInnerDataWasComputedBy(signingAdministratorHint) {
                        return true
                    }
                }
                assertionFailure("In practice, the signingAdministratorHint should always work.")
        ...
        public static func decryptAndCheckIntegrity(encryptedAdministratorChain: EncryptedData, blobMainSeed: Seed, expectedGroupUID: UID, alreadyTrustedPrefixAdministratorChain: GroupV2.AdministratorsChain?) throws -> AdministratorsChain {
            ...
            let administratorChainWithCheckedIntegrity = try administratorChain.withCheckedIntegrity(expectedGroupUID: expectedGroupUID, alreadyTrustedPrefixAdministratorChain: alreadyTrustedPrefixAdministratorChain)
        ...
        public var allCurrentAdministratorIdentities: [ObvCryptoIdentity] {
            blocks.last?.allAdministratorIdentities ?? [ObvCryptoIdentity]()
        }
        ...
        public var anAdministratorWasDemotedInTheLastUpdate: Bool {
            ...
            return !Set(lastBlocks[0].allAdministratorIdentities).subtracting(lastBlocks[1].allAdministratorIdentities).isEmpty
        }
        ...
        fileprivate func withCheckedIntegrity(expectedGroupUID: UID, alreadyTrustedPrefixAdministratorChain: AdministratorsChain?) throws -> AdministratorsChain {
            
            guard !self.integrityChecked else { return self }
            
            // If there is an already prefix trusted chain, use it
            
            if let alreadyTrustedPrefixAdministratorChain, alreadyTrustedPrefixAdministratorChain.numberOfBlocks > 0 {
                
                // Make sure the trusted chain is a prefix of this chain
                
                guard alreadyTrustedPrefixAdministratorChain.isPrefixOfOtherAdministratorsChain(self) else {
                    assertionFailure()
                    throw Self.makeError(message: "Not a prefix")
                }
                
                // Check the remaining blocks
                
                if alreadyTrustedPrefixAdministratorChain.numberOfBlocks < self.numberOfBlocks {
                    
                    guard var lastValidBlock = alreadyTrustedPrefixAdministratorChain.blocks.last else {
                        assertionFailure()
                        throw Self.makeError(message: "Invalid blocks")
                    }
                    var administratorsOfLastValidBlock = lastValidBlock.innerData.encodedAdministrators.compactMap({ ObvCryptoIdentity($0) })

                    for block in self.blocks[alreadyTrustedPrefixAdministratorChain.numberOfBlocks..<self.numberOfBlocks] {
                        
                        // Check the chain of digests
                        
                        guard lastValidBlock.computeSha256() == block.innerData.previousBlockHash else {
                            assertionFailure()
                        ...
                        guard let signingAdministratorHint = block.allAdministratorIdentities.first else {
                            throw Self.makeError(message: "Invalid block signature")
                        }
                        guard block.signatureOnInnerDataIsValid(administratorsOfPreviousBlock: administratorsOfLastValidBlock, signingAdministratorHint: signingAdministratorHint) else {
                            throw Self.makeError(message: "Invalid block signature")
                        }
                        
                        // Prepare for the next iteration
                        
                        administratorsOfLastValidBlock = block.innerData.encodedAdministrators.compactMap({ ObvCryptoIdentity($0) })
                ...
                // If we reach this point, the integrity of the chain has been checked with the help of the trusted prefix
                
                return AdministratorsChain(groupUID: self.groupUID, blocks: self.blocks, integrityChecked: true)

            }
            
            // If we reach this point, we have no prefix allowing to speed-up the chain integrity
            ...
            for block in blocks {
                ...
                guard block.signatureOnInnerDataIsValid(administratorsOfPreviousBlock: administratorsOfPreviousBlock, signingAdministratorHint: signingAdministratorHint) else {
                    throw Self.makeError(message: "Invalid block signature")
                }
    ...
    public struct ServerBlob: ObvFailableCodable, ObvErrorMaker {
        ...
        public static func decryptThenCheckSignature(encryptedServerBlob: EncryptedData, blobMainSeed: Seed, blobVersionSeed: Seed, expectedGroupIdentifier: Identifier, alreadyTrustedPrefixAdministratorChain: GroupV2.AdministratorsChain?, solveChallengeDelegate: ObvSolveChallengeDelegate) throws -> (blob: ServerBlob, signer: ObvCryptoIdentity) {
            ...
            let checkedAdministratorsChain = try blob.administratorsChain.withCheckedIntegrity(expectedGroupUID: expectedGroupIdentifier.groupUID, alreadyTrustedPrefixAdministratorChain: alreadyTrustedPrefixAdministratorChain)
        ...
        public func withCheckedAdministratorsChainIntegrity(expectedGroupIdentifier: Identifier, alreadyTrustedPrefixAdministratorChain: GroupV2.AdministratorsChain?) throws -> ServerBlob {
            
            let checkedAdministratorsChain = try self.administratorsChain.withCheckedIntegrity(expectedGroupUID: expectedGroupIdentifier.groupUID, alreadyTrustedPrefixAdministratorChain: alreadyTrustedPrefixAdministratorChain)
