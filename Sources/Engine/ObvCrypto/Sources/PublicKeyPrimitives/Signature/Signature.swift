...
protocol SignatureCommon {
    static func sign(_: Data, with: PrivateKeyForSignature, and: PublicKeyForSignature, using: PRNG) -> Data?
...
protocol SignatureConcrete: SignatureCommon {
    ...
    static func generateKeyPair(with: PRNG) -> (PublicKeyForSignature, PrivateKeyForSignature)
}

protocol SignatureGeneric: SignatureCommon {
    static func generateKeyPair(for: SignatureImplementationByteId, with: PRNG) -> (PublicKeyForSignature, PrivateKeyForSignature)
}
...
public final class Signature: SignatureGeneric {
    ...
    static func generateKeyPair(for implemByteId: SignatureImplementationByteId, with prng: PRNG) -> (PublicKeyForSignature, PrivateKeyForSignature) {
        return implemByteId.algorithmImplementation.generateKeyPair(with: prng)
    }
    
    static func sign(_ data: Data, with privateKey: PrivateKeyForSignature, and publicKey: PublicKeyForSignature, using prng: PRNG) -> Data? {
...
extension SignatureECSDSA256overEdwardsCurve {
    ...
    static func generateKeyPair(with prng: PRNG) -> (PublicKeyForSignature, PrivateKeyForSignature) {
    ...
    static func sign(_ message: Data, with privateKey: PrivateKeyForSignature, and publicKey: PublicKeyForSignature, using prng: PRNG) -> Data? {
    ...
    private static func sign<H: HashFunction>(_ message: Data, with _privateKey: PrivateKeyForSignature, and _publicKey: PublicKeyForSignature, using prng: PRNG, hashFunction: H.Type) -> Data? {
