...
public final class ObvEngine: ObvManager {
    ...
    lazy var logger = Logger(subsystem: logSubsystem, category: "ObvEngine")
...
extension ObvEngine {
    ...
    public func isOwnedIdentityKeycloakManaged(_ ownedCryptoId: ObvCryptoId) async throws -> Bool {
        let flowId = FlowIdentifier()
        return try await isOwnedIdentityKeycloakManaged(ownedIdentity: ownedCryptoId.cryptoIdentity, flowId: flowId)
    }
    ...
    public func requestRegisterToPushNotificationsForAllActiveOwnedIdentities(deviceTokens: (pushToken: Data, voipToken: Data?)?, defaultDeviceNameForFirstRegistration: String) async throws {
        ...
        ObvDisplayableLogs.shared.log("[🚩][\(flowId.shortDebugDescription)] ObvEngine.requestRegisterToPushNotificationsForAllActiveOwnedIdentities(deviceTokens:defaultDeviceNameForFirstRegistration:)")
    ...
    public func saveKeycloakAuthState(with ownedCryptoId: ObvCryptoId, rawAuthState: Data) async throws {
        guard let createContextDelegate else { throw ObvError.createContextDelegateIsNil }
        guard let identityDelegate else { throw ObvError.identityDelegateIsNil }
    ...
    public func unbindOwnedIdentityFromKeycloak(ownedCryptoId: ObvCryptoId, isUnbindRequestByUser: Bool) async throws(ObvUnbindOwnedIdentityFromKeycloakError) {
        ...
        do {
            ...
            // If the unbind was requested by the user, we make sure that the keycloak does not enforce a restriction on the transfer.
            // When this is the case, the user is not allowed to unbind. This check is also performed at the protocol level, this test
            ...
            let message = try protocolDelegate.getOwnedIdentityKeycloakUnbindingMessage(ownedCryptoIdentity: ownedCryptoId.cryptoIdentity, isUnbindRequestByUser: isUnbindRequestByUser)
            let flowId = try flowDelegate.startBackgroundActivityForStartingOrResumingProtocol()
            ...
            // If we reach this point, the protocol message was processed (i.e., deleted from database)
            // It does not necessarily mean that the protocol was a success.
            // So we check the identity is indeed unbound from keycloak
            ...
            ObvEngineNotificationNew.ownedIdentityUnbindingFromKeycloakPerformed(ownedIdentity: ownedCryptoId)
                .postOnBackgroundQueue(within: appNotificationCenter)

        } catch {
                        
            if let error = error as? ObvUnbindOwnedIdentityFromKeycloakError {
                throw error
            } else {
    ...
    /// This only concerns a keycloak managed identity. If `isTransferRestricted` is true, the user will be unable to transfer their identity
    /// to a new device unless they can successfully authenticate with the Keycloak server
    public func setIsTransferRestricted(to isTransferRestricted: Bool, ownedCryptoId: ObvCryptoId) async throws {
        guard let createContextDelegate else { throw ObvError.createContextDelegateIsNil }
        guard let identityDelegate else { throw ObvError.identityDelegateIsNil }
        let log = self.log
        let flowId = FlowIdentifier()
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, any Error>) in
            queueForSynchronizingCallsToManagers.async {
                createContextDelegate.performBackgroundTaskAndWait(flowId: flowId) { obvContext in
                    do {
                        try identityDelegate.setIsTransferRestricted(to: isTransferRestricted, ownedCryptoId: ownedCryptoId, within: obvContext)
                        try obvContext.save(logOnFailure: log)
...
extension ObvEngine {
    ...
    public func getAllDialogsWithinEngine() async throws -> [ObvDialog] {
        guard let createContextDelegate else { throw ObvError.createContextDelegateIsNil }
        let randomFlowId = FlowIdentifier()
        return try await withCheckedThrowingContinuation { [weak self] (continuation: CheckedContinuation<[ObvDialog], Error>) in
    ...
    public func respondTo(_ obvDialog: ObvDialog) async throws {
        ...
        // Responding to an ObvDialog is a critical long-running task, so we always extend the app runtime to make sure that responding to a dialog
        // (and all the resulting network exchanges) eventually finish, even if the app moves to the background between the call to this method and
        // the moment the data is actually sent to the server.
...
extension ObvEngine {
    ...
    public func startTrustEstablishmentProtocolOfRemoteIdentity(with remoteCryptoId: ObvCryptoId, withFullDisplayName remoteFullDisplayName: String, forOwnedIdentyWith ownedCryptoId: ObvCryptoId) throws {
        ...
        let obvOwnedIdentity = try getOwnedIdentity(with: ownedCryptoId)
...
extension ObvEngine {
    ...
    public func generateReturnReceiptElements() -> ObvReturnReceiptElements {
    ...
    public func decryptPayloadOfObvReturnReceipt(_ obvReturnReceipt: ObvEncryptedReceivedReturnReceipt, decryptionKeyCandidates: Set<Data>) throws -> ObvDecryptedReceivedReturnReceipt? {
        return try returnReceiptSender.decryptPayloadOfObvReturnReceipt(obvReturnReceipt, decryptionKeyCandidates: decryptionKeyCandidates)
    }
...
extension ObvEngine {
    ...
    ///   - extendedPayload:
    ///   - isVoipMessageForStartingCall:
    ///   - contactDeviceIdentifiersToExclude:
    ///   - callCompletionWhenMessageAndAttachmentsAreSent: If `true`, the completion handler is called after the server acknowledgement of the message and all of its attachments. Otherwise, the completion is called after the message is sent, and the attachments scheduled (this was the default).
    /// - Returns: 
    public func post(messagePayload: Data, extendedPayload: Data?, withUserContent: Bool, isVoipMessageForStartingCall: Bool, attachmentsToSend: [ObvAttachmentToSend], toContactIdentitiesWithCryptoId contactCryptoIds: Set<ObvCryptoId>, ofOwnedIdentityWithCryptoId ownedCryptoId: ObvCryptoId, contactDeviceIdentifiersToExclude: Set<ObvContactDeviceIdentifier> = Set([]), alsoPostToOtherOwnedDevices: Bool, callCompletionWhenMessageAndAttachmentsAreSent: Bool = false, completionHandler: (() -> Void)? = nil) throws -> [ObvCryptoId: Data] {
        ...
        try createContextDelegate.performBackgroundTaskAndWaitOrThrow(flowId: flowId) { [weak self] obvContext in
            ...
            try messageIdentifiersForToIdentities.keys.forEach { messageId in
                ...
                try flowDelegate.addBackgroundActivityForPostingApplicationMessageAttachmentsWithinFlow(withFlowId: flowId,
                                                                                                        ...
                                                                                                        waitUntilMessageAndAttachmentsAreSent: callCompletionWhenMessageAndAttachmentsAreSent)
            }
    ...
    public func post(messagePayload: Data, toContactDevice contactDeviceIdentifier: ObvContactDeviceIdentifier, completionHandler: (() -> Void)? = nil) throws -> ObvMessageIdentifier {
        ...
        let message = ObvChannelApplicationMessageToSend(toContactDevice: contactDeviceIdentifier, messagePayload: messagePayload)
        
        let flowId = try flowDelegate.startNewFlow(completionHandler: completionHandler)

        var messageIdentifier: ObvMessageIdentifier?
        ...
        try createContextDelegate.performBackgroundTaskAndWaitOrThrow(flowId: flowId) { [weak self] obvContext in
            ...
            guard let contacts = messageIdentifiersForToIdentities[messageId], contacts.count == 1, contacts.first == contactDeviceIdentifier.contactCryptoId.cryptoIdentity  else {
                assertionFailure()
        ...
        guard let messageIdentifier else {
...
extension ObvEngine {
    ...
    public func decrypt(encryptedPushNotification encryptedNotification: ObvEncryptedRemoteUserNotification) async throws -> ObvDecryptedNotification {
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<ObvDecryptedNotification, Error>) in
            let randomFlowId = FlowIdentifier()
            createContextDelegate.performBackgroundTask(flowId: randomFlowId) { (obvContext) in
                do {
                    ...
                    let receivedApplicationOrProtocolMessage = try channelDelegate.decryptUserNotification(encryptedMessage, within: randomFlowId)
                    
                    switch receivedApplicationOrProtocolMessage {
                    ...
                    case .applicationMessage(let message):
                        ...
                        if message.messageId.ownedCryptoIdentity == message.remoteCryptoIdentity {
                            let obvOwnedMessage = ObvOwnedMessage(messageId: message.messageId,
                                                                  ...
                                                                  deviceUID: message.remoteDeviceUid)
                            ...
                            let obvMessage = ObvMessage(fromContactIdentity: fromContactIdentity,
                                                        ...
                                                        messagePayload: message.messagePayload,
                                                        extendedMessagePayload: message.extendedMessagePayload)
                            decryptedNotification = .obvMessageOrObvOwnedMessage(.obvMessage(obvMessage))
                        }

                    case .protocolMessage(let receivedProtocolMessage):
                        
                        let protocolMessage = try ObvProtocolMessage(receivedProtocolMessage: receivedProtocolMessage)
                        decryptedNotification = .protocolMessage(protocolMessage)
                        
                    }
                    
                    continuation.resume(returning: decryptedNotification)
...
extension ObvEngine {
    ...
    public func downloadMessagesAndConnectWebsockets() async throws {
        ...
        let activeOwnedCryptoIdsAndCurrentDeviceUIDs = try await getActiveOwnedIdentitiesAndCurrentDeviceUids()
        do {
    ...
    private func getActiveOwnedIdentitiesAndCurrentDeviceUids() async throws -> Set<OwnedCryptoIdentityAndCurrentDeviceUID> {
        guard let identityDelegate  else { assertionFailure(); throw ObvError.identityDelegateIsNil }
        guard let createContextDelegate else { assertionFailure(); throw ObvError.createContextDelegateIsNil }
...
extension ObvEngine {
    ...
    public func initiateOwnedIdentityTransferProtocolOnSourceDevice(ownedCryptoId: ObvCryptoId, onAvailableSessionNumber: @MainActor @escaping (ObvOwnedIdentityTransferSessionNumber) -> Void, onAvailableSASExpectedOnInput: @MainActor @escaping (ObvOwnedIdentityTransferSas, String, UID) -> Void) async throws {
    ...
    public func appIsShowingSasAndExpectingEndOfProtocol(protocolInstanceUID: UID, onSyncSnapshotReception: @escaping () -> Void, onSuccessfulTransfer: @escaping (ObvCryptoId, Error?) -> Void, onKeycloakAuthenticationNeeded: @escaping (ObvCryptoId, ObvKeycloakConfiguration, ObvKeycloakTransferProofElements) -> Void) async {
    ...
    public func userEnteredValidSASOnSourceDeviceForOwnedIdentityTransferProtocol(enteredSAS: ObvOwnedIdentityTransferSas, isTransferRestricted: Bool, deviceToKeepActive: UID?, ownedCryptoId: ObvCryptoId, protocolInstanceUID: UID, snapshotSentToTargetDevice: @escaping () -> Void) async throws {
        guard let protocolDelegate else { throw ObvError.protocolDelegateIsNil }
    ...
    public func userProvidesProofOfAuthenticationOnKeycloakServer(ownedCryptoId: ObvCryptoId, protocolInstanceUID: UID, proof: ObvKeycloakTransferProofAndAuthState) async throws {
        guard let protocolDelegate else { throw ObvError.protocolDelegateIsNil }
        try await protocolDelegate.continueOwnedIdentityTransferProtocolOnUserProvidesProofOfAuthenticationOnKeycloakServer(
            ownedCryptoId: ownedCryptoId,
            protocolInstanceUID: protocolInstanceUID,
            proof: proof)
    }
...
extension ObvEngine {
    ...
    public func requestPropagationToOtherOwnedDevicesOfAllOwnedIdentities(of syncAtom: ObvSyncAtom) async throws {
        ...
        let ownedCryptoIds = try await getActiveOwnedIdentitiesAndCurrentDeviceUids().map(\.ownedCryptoId)
        
        guard !ownedCryptoIds.isEmpty else { return }
        
        var ownedCryptoIdsWithOtherOwnedDevices = [ObvCryptoIdentity]()
        for ownedCryptoId in ownedCryptoIds {
            let otherDeviceUidsOfOwnedIdentity = try await getOtherOwnedDeviceUidsOfOwnedIdentity(ownedCryptoId)
            if !otherDeviceUidsOfOwnedIdentity.isEmpty {
                ownedCryptoIdsWithOtherOwnedDevices.append(ownedCryptoId)
            }
        }
        
        guard !ownedCryptoIdsWithOtherOwnedDevices.isEmpty else { return }
        
        let flowId = try flowDelegate.startBackgroundActivityForStartingOrResumingProtocol()

        for ownedCryptoId in ownedCryptoIdsWithOtherOwnedDevices {
            do {
    ...
    private func getOtherOwnedDeviceUidsOfOwnedIdentity(_ ownedCryptoId: ObvCryptoIdentity) async throws -> Set<UID> {
        guard let createContextDelegate else { throw ObvError.createContextDelegateIsNil }
        guard let identityDelegate else { throw ObvError.identityDelegateIsNil }
...
extension ObvEngine {
    ...
    enum ObvError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .unexpectedNumberOfMessageIdentifiers:
                return "Unexpected number of message identifiers"
            case .unexpectedContactIdentifier:
