    ...
    static func getTrustOriginsOfContactIdentity(with contactCryptoId: ObvCryptoId, ofOwnedIdentyWith ownedCryptoId: ObvCryptoId, using identityDelegate: ObvIdentityDelegate, within obvContext: ObvContext) throws -> [ObvTrustOrigin] {
        ...
        let obvTrustOrigins = trustOrigins.map { (trustOrigin) -> ObvTrustOrigin in
            ...
            case .direct(timestamp: let timestamp):
                return ObvTrustOrigin.direct(contactIdentifier: contactIdentifier, timestamp: timestamp)
...
//                                                    within: obvContext)
                return ObvTrustOrigin.group(contactIdentifier: contactIdentifier, timestamp: timestamp, groupOwner: ObvCryptoId(cryptoIdentity: groupOwnerCryptoIdentity))
...
//                                                  identityDelegate: identityDelegate,
//                                                  within: obvContext)
                return ObvTrustOrigin.introduction(contactIdentifier: contactIdentifier, timestamp: timestamp, mediator: ObvCryptoId(cryptoIdentity: mediatorCryptoIdentity))

            case .keycloak(timestamp: let timestamp, keycloakServer: let keycloakServer):
                return ObvTrustOrigin.keycloak(contactIdentifier: contactIdentifier, timestamp: timestamp, keycloakServer: keycloakServer)
                
            case .serverGroupV2(timestamp: let timestamp, groupIdentifier: let groupIdentifier):
                return ObvTrustOrigin.serverGroupV2(contactIdentifier: contactIdentifier, timestamp: timestamp, groupIdentifier: groupIdentifier)
