...
extension ObvChannelManagerImplementation {
    ...
    public func decryptUserNotification(_ receivedMessage: ObvNetworkReceivedMessageEncrypted, within flowId: FlowIdentifier) throws -> ReceivedApplicationOrProtocolMessage {
        guard let contextCreator = self.contextCreator else {
            os_log("The context creator is not set", log: log, type: .fault)
            assertionFailure()
            throw ObvChannelManagerImplementation.makeError(message: "The context creator is not set")
        }
        var applicationOrProtocolMessage: ReceivedApplicationOrProtocolMessage?
        try contextCreator.performBackgroundTaskAndWaitOrThrow(flowId: flowId) { obvContext in
            // Since we do not save the context, we do not need to wait until a slot is available
            applicationOrProtocolMessage = try delegateManager.networkReceivedMessageDecryptorDelegate.decryptUserNotification(receivedMessage, within: obvContext)
        ...
        guard let message = applicationOrProtocolMessage else {
