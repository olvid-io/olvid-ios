...
actor ServerQueryWebSocketCoordinator: ServerQueryWebSocketDelegate {
    ...
    init(logPrefix: String, prng: PRNGService) {
    ...
    func handleServerQuery(pendingServerQueryObjectId: NSManagedObjectID, flowId: FlowIdentifier) async throws {
        ...
        let protocolInstanceUIDOfWebSocketToClose: UID?
        let serverResponseTypeToSet: ServerResponse.ResponseType? // If nil after the switch, the PendingServerQuery is deleted from DB
        
        switch pendingServerQueryType {
        ...
        case .sourceGetSessionNumber(protocolInstanceUID: let protocolInstanceUID):
            do {
                ...
                protocolInstanceUIDOfWebSocketToClose = nil
                serverResponseTypeToSet = .sourceGetSessionNumberMessage(result: .requestSucceeded(sourceConnectionId: sourceConnectionId, sessionNumber: sessionNumber))
                
            } catch {

                protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
                serverResponseTypeToSet = .sourceGetSessionNumberMessage(result: .requestFailed)
        ...
        case .sourceWaitForTargetConnection(protocolInstanceUID: let protocolInstanceUID):
            ...
            do {
                ...
                protocolInstanceUIDOfWebSocketToClose = nil
                serverResponseTypeToSet = .sourceWaitForTargetConnection(result: .requestSucceeded(targetConnectionId: targetConnectionId, payload: payload))
                
            } catch {
                
                protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
                serverResponseTypeToSet = .sourceWaitForTargetConnection(result: .requestFailed)
        ...
        case .targetSendEphemeralIdentity(protocolInstanceUID: let protocolInstanceUID, transferSessionNumber: let transferSessionNumber, payload: let payload):
            ...
            do {
                ...
                case .success((let otherConnectionId, let payload)):
                    ...
                    protocolInstanceUIDOfWebSocketToClose = nil
                    serverResponseTypeToSet = .targetSendEphemeralIdentity(result: .requestSucceeded(otherConnectionId: otherConnectionId, payload: payload))
                ...
                case .failure:
                    ...
                    protocolInstanceUIDOfWebSocketToClose = nil
                    serverResponseTypeToSet = .targetSendEphemeralIdentity(result: .incorrectTransferSessionNumber)
                ...
                protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
                serverResponseTypeToSet = .targetSendEphemeralIdentity(result: .requestDidFail)
                
                closeCachedWebSocket(protocolInstanceUID: protocolInstanceUID)
        ...
        case .transferRelay(protocolInstanceUID: let protocolInstanceUID, connectionIdentifier: let connectionIdentifier, payload: let payload, thenCloseWebSocket: let thenCloseWebSocket):
            ...
            do {
                ...
                protocolInstanceUIDOfWebSocketToClose = thenCloseWebSocket ? protocolInstanceUID : nil
                serverResponseTypeToSet = .transferRelay(result: .requestSucceeded(payload: responsePayload))
                
            } catch {
                
                protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
                serverResponseTypeToSet = .transferRelay(result: .requestFailed)
        ...
        case .transferWait(protocolInstanceUID: let protocolInstanceUID, connectionIdentifier: let connectionIdentifier):
            ...
            do {
                ...
                protocolInstanceUIDOfWebSocketToClose = nil
                serverResponseTypeToSet = .transferWait(result: .requestSucceeded(payload: responsePayload))

            } catch {
                
                protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
                serverResponseTypeToSet = .transferWait(result: .requestFailed)
        ...
        case .closeWebsocketConnection(protocolInstanceUID: let protocolInstanceUID):
            ...
            protocolInstanceUIDOfWebSocketToClose = protocolInstanceUID
            serverResponseTypeToSet = nil
                
        }
        
        // Process the serverResponseTypeToSet and the protocolInstanceUIDOfWebSocketToClose
        
        if let protocolInstanceUIDOfWebSocketToClose {
            closeCachedWebSocket(protocolInstanceUID: protocolInstanceUIDOfWebSocketToClose)
        }
        ...
        if let serverResponseTypeToSet {
            ...
            do {
                ...
                assertionFailure()
                throw ObvError.failedToSetWebSocketPendingServerQueryResponse
            }
        } else {
            ...
            do {
                ...
                assertionFailure()
                throw ObvError.failedToDeleteWebSocketPendingServerQuery
            }
        }
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<ServerQuery.QueryType?, Error>) in
            contextCreator.performBackgroundTask(flowId: flowId) { obvContext in
                do {
                    ...
                    return continuation.resume(returning: pendingServerQuery.queryType)
                } catch {
    ...
    enum ObvError: Error {
        ...
        case overflow
        case failedToSetWebSocketPendingServerQueryResponse
        case channelDelegateIsNil
        case failedToDeleteWebSocketPendingServerQuery
    }
