...
enum Expectation: Equatable, Hashable, CustomDebugStringConvertible {
    ...
    case deletedOutboxMessageWasCreated(messageId: ObvMessageIdentifier) // A DeletedOutboxMessage is created just after a message and its attachments are sent
    ...
    static func == (lhs: Expectation, rhs: Expectation) -> Bool {
        ...
        case .deletedOutboxMessageWasCreated(messageId: let id1):
            switch rhs {
            case .deletedOutboxMessageWasCreated(messageId: let id2):
                return id1 == id2
            default:
                return false
            }
    ...
    var debugDescription: String {
        switch self {
        case .deletedOutboxMessageWasCreated(messageId: let uid):
            return "deletedOutboxMessageWasCreated<\(uid.debugDescription)>"
