...
final class ProtocolStarterCoordinator: ProtocolStarterDelegate {
    ...
    private static var logger = Logger(subsystem: "io.olvid.protocol", category: ProtocolStarterCoordinator.logCategory)
    ...
    public func finalizeInitialization(flowId: FlowIdentifier, runningLog: RunningLogError) {
        Self.logger = Logger(subsystem: delegateManager.logSubsystem, category: ProtocolStarterCoordinator.logCategory)
...
extension ProtocolStarterCoordinator {
    ...
    func continueOwnedIdentityTransferProtocolOnUserProvidesProofOfAuthenticationOnKeycloakServer(ownedCryptoId: ObvTypes.ObvCryptoId, protocolInstanceUID: ObvCrypto.UID, proof: ObvTypes.ObvKeycloakTransferProofAndAuthState) async throws {

        let coreProtocolMessage = CoreProtocolMessage(
            channelType: .local(ownedIdentity: ownedCryptoId.cryptoIdentity),
            cryptoProtocolId: .ownedIdentityTransfer,
            protocolInstanceUid: protocolInstanceUID)
        let message = OwnedIdentityTransferProtocol.KeycloakAuthenticationProofMessage(
            coreProtocolMessage: coreProtocolMessage,
            proof: proof)

        guard let initialMessageToSend = message.generateObvChannelProtocolMessageToSend(with: prng) else {
            Self.logger.fault("Could create generic protocol message to send")
            throw makeError(message: "Could create generic protocol message to send")
        }

        // We don't need to listen yo any notification here. We are already listening to the appropriate notifications as we previously executed
        // ``appIsShowingSasAndExpectingEndOfProtocol(protocolInstanceUID:onSyncSnapshotReception:onSuccessfulTransfer:onKeycloakAuthenticationNeeded:)``
    ...
    func appIsShowingSasAndExpectingEndOfProtocol(protocolInstanceUID: ObvCrypto.UID, onSyncSnapshotReception: @escaping () -> Void, onSuccessfulTransfer: @escaping (ObvTypes.ObvCryptoId, (any Error)?) -> Void, onKeycloakAuthenticationNeeded: @escaping (ObvTypes.ObvCryptoId, ObvTypes.ObvKeycloakConfiguration, ObvTypes.ObvKeycloakTransferProofElements) -> Void) async {
        ...
        do {
            var token: NSObjectProtocol?
            token = notificationDelegate.addObserverOfOwnedIdentityTransferProtocolNotification(.keycloakAuthenticationRequiredAsProfileIsTransferRestricted { payload in
                // Make sure the received notification concerns the protocol we launched here.
                guard payload.protocolInstanceUID == protocolInstanceUID else { return }
                let ownedCryptoId = ObvCryptoId(cryptoIdentity: payload.ownedCryptoIdentity)
                // Transfer the information to the app
                onKeycloakAuthenticationNeeded(ownedCryptoId, payload.keycloakConfiguration, payload.keycloakTransferProofElements)
            })
            localTokens.append(token!)
        }
    ...
    func continueOwnedIdentityTransferProtocolOnUserEnteredSASOnSourceDevice(enteredSAS: ObvOwnedIdentityTransferSas, isTransferRestricted: Bool, deviceToKeepActive: UID?, ownedCryptoId: ObvCryptoId, protocolInstanceUID: UID, snapshotSentToTargetDevice: @escaping () -> Void) async throws {
        ...
        let coreMessage = CoreProtocolMessage(channelType: .local(ownedIdentity: ownedCryptoId.cryptoIdentity),
                                              cryptoProtocolId: .ownedIdentityTransfer,
                                              protocolInstanceUid: protocolInstanceUID)
        let message = OwnedIdentityTransferProtocol.SourceSASInputMessage(coreProtocolMessage: coreMessage,
                                                                          enteredSAS: enteredSAS,
                                                                          isTransferRestricted: isTransferRestricted,
                                                                          deviceUIDToKeepActive: deviceToKeepActive)
        guard let initialMessageToSend = message.generateObvChannelProtocolMessageToSend(with: prng) else {
            os_log("Could create generic protocol message to send", log: log, type: .fault)
            throw makeError(message: "Could create generic protocol message to send")
        }
    ...
    func getOwnedIdentityKeycloakUnbindingMessage(ownedCryptoIdentity: ObvCryptoIdentity, isUnbindRequestByUser: Bool) throws -> ObvChannelProtocolMessageToSend {
        ...
        let coreMessage = CoreProtocolMessage(channelType: .local(ownedIdentity: ownedCryptoIdentity),
                                              cryptoProtocolId: .keycloakBindingAndUnbinding,
                                              protocolInstanceUid: protocolInstanceUid)
        let initialMessage = KeycloakBindingAndUnbindingProtocol.OwnedIdentityKeycloakUnbindingMessage(coreProtocolMessage: coreMessage, isUnbindRequestByUser: isUnbindRequestByUser)
        guard let initialMessageToSend = initialMessage.generateObvChannelProtocolMessageToSend(with: prng) else {
            os_log("Could create generic protocol message to send", log: log, type: .fault)
            throw makeError(message: "Could create generic protocol message to send")
        }
