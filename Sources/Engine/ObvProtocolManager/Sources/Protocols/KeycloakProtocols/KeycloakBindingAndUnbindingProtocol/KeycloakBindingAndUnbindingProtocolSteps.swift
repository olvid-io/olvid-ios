...
extension KeycloakBindingAndUnbindingProtocol {
    ...
    class OwnedIdentityKeycloakUnbindingStep: ProtocolStep {
        ...
        override func executeStep(within obvContext: ObvContext) throws -> ConcreteProtocolState? {
            ...
            switch receivedMessage {
            case .ownedIdentityKeycloakUnbinding(receivedMessage: let message):
                propagationNeeded = true
                isUnbindRequestByUser = message.isUnbindRequestByUser
            case .propagateKeycloakUnbinding:
                propagationNeeded = false
                isUnbindRequestByUser = false
            }
            ...
            do {
                try identityDelegate.unbindOwnedIdentityFromKeycloak(
                    ownedCryptoIdentity: ownedIdentity,
                    isUnbindRequestByUser: isUnbindRequestByUser,
                    within: obvContext)
            } catch {
                // If the unbind fails, we properly finish, as the engine waits for the protocol message to be deleted to continue
                return FinishedState()
            }
