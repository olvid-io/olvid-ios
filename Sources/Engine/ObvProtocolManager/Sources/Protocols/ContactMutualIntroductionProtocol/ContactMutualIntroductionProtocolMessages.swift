...
extension ContactMutualIntroductionProtocol {
    ...
    struct MediatorInvitationMessage: ConcreteProtocolMessage {
        ...
        // Initializer used when parsing a GenericReceivedProtocolMessage obtained from a decrypted notification
        
        init(withGenericReceivedProtocolMessage message: GenericReceivedProtocolMessage) throws(ObvError) {
            self.coreProtocolMessage = CoreProtocolMessage(with: message)
            guard self.coreProtocolMessage.cryptoProtocolId == ContactMutualIntroductionProtocol.id else {
                throw .unexpectedCryptoProtocolId
            }
            guard message.protocolMessageRawId == self.id.rawValue else {
                throw .unexpectedProtocolMessageId
            }
            let encodedContactIdentityCoreDetails: Data
            do {
                (contactIdentity, encodedContactIdentityCoreDetails) = try message.encodedInputs.obvDecode()
        ...
        enum ObvError: Error {
            ...
            case unexpectedProtocolMessageId
