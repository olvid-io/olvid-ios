...
//        case targetGetSessionNumber = 5
        ...
        case sourceWaitForKeycloakAuthenticationProof = 13
        case keycloakAuthenticationProof = 98
        ...
        var concreteProtocolMessageType: ConcreteProtocolMessage.Type {
            ...
            case .keycloakAuthenticationProof:
                return KeycloakAuthenticationProofMessage.self
            case .sourceWaitForKeycloakAuthenticationProof:
                return SourceWaitForKeycloakAuthenticationProofMessage.self
            }
        }
    ...
    struct SourceSASInputMessage: ConcreteProtocolMessage {
        ...
        init(coreProtocolMessage: CoreProtocolMessage, enteredSAS: ObvOwnedIdentityTransferSas, isTransferRestricted: Bool, deviceUIDToKeepActive: UID?) {
            self.coreProtocolMessage = coreProtocolMessage
        ...
        var encodedInputs: [ObvEncoded] {
            var encoded = [enteredSAS.obvEncode(), isTransferRestricted.obvEncode()]
    ...
    // MARK: - SourceKeycloakAuthenticationProofMessage
    
    /// Requests a Keycloak authentication proof from the target device as part of a server query.
    /// This message is sent when transferring a Keycloak-managed profile with transfer protection enabled (`isTransferRestricted` = `true`).
    /// The response, also using this structure, is expected to be sent back by the network manager, where the `result` field will indicate the outcome.
    struct SourceWaitForKeycloakAuthenticationProofMessage: ConcreteProtocolMessage {
        
        let id: ConcreteProtocolMessageId = MessageId.sourceWaitForKeycloakAuthenticationProof
        let coreProtocolMessage: CoreProtocolMessage

        // Not used when posting this message from the protocol manager
        let result: SourceWaitForKeycloakAuthenticationProofResult
    ...
    // MARK: - KeycloakAuthenticationProofMessage
    
    struct KeycloakAuthenticationProofMessage: ConcreteProtocolMessage {
        
        let id: ConcreteProtocolMessageId = MessageId.keycloakAuthenticationProof
        let coreProtocolMessage: CoreProtocolMessage

        let proof: ObvKeycloakTransferProofAndAuthState
        
        // Init when sending this message

        init(coreProtocolMessage: CoreProtocolMessage, proof: ObvKeycloakTransferProofAndAuthState) {
            self.coreProtocolMessage = coreProtocolMessage
            self.proof = proof
        }
