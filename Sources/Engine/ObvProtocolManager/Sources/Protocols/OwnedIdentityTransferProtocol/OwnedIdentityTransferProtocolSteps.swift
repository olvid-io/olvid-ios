        ...
        override func executeStep(within obvContext: ObvContext) throws -> ConcreteProtocolState? {
            ...
            do {
                ...
                case .no:
                    ...
                    let syncSnapshotAsObvDict = try syncSnapshotDelegate.getSyncSnapshotNodeAsObvDictionary(context: .transfer(ownedCryptoId: ObvCryptoId(cryptoIdentity: ownedIdentity)))
                    let cleartext: Data
                ...
                case .requestSucceeded(let payload):
                    ...
                    if let encoded = ObvEncoded(withRawData: cleartextPayload),
                       ...
                       let obvDictionary = ObvDictionary(listOfEncoded[0]) {
                        ...
                        let syncSnapshot = try syncSnapshotDelegate.decodeSyncSnapshot(from: obvDictionary, context: .transfer(ownedCryptoId: ObvCryptoId(cryptoIdentity: ownedIdentity)))
                        ...
                        // Leverage the OwnedIdentityRestorationHelper to restore the transferred
                        // owned identity
                        
                        let helper = OwnedIdentityRestorationHelper(
                            identityNode: syncSnapshot.identityNode,
                            currentDeviceName: currentDeviceName,
                        ...
                        let nonDefinitiveErrors: [Error]
                        do {
                            try helper.performRestoration(within: obvContext)
                            nonDefinitiveErrors = []
                        } catch {
                            switch error {
                            case .definitiveError(error: let error):
                                throw error
                            case .nonDefinitiveErrors(errors: let errors):
                                nonDefinitiveErrors = errors
                            }
                        }
