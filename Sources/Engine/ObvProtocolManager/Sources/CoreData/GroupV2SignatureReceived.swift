...
@objc(GroupV2SignatureReceived)
final class GroupV2SignatureReceived: NSManagedObject, ObvManagedObject {
        
    // MARK: Internal constants

    private static let entityName = "GroupV2SignatureReceived"
    ...
    convenience init?(ownedCryptoIdentity: ObvCryptoIdentity, signature: Data, within obvContext: ObvContext) {
        ...
        let entityDescription = NSEntityDescription.entity(forEntityName: GroupV2SignatureReceived.entityName, in: obvContext)!
...
extension GroupV2SignatureReceived {
    
    @nonobjc class func fetchRequest() -> NSFetchRequest<GroupV2SignatureReceived> {
        return NSFetchRequest<GroupV2SignatureReceived>(entityName: GroupV2SignatureReceived.entityName)
    }
