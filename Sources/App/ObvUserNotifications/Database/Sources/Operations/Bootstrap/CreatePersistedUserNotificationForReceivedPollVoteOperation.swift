...
public final class CreatePersistedUserNotificationForReceivedPollVoteOperation: ContextualOperationWithSpecificReasonForCancel<CoreDataOperationReasonForCancel>, @unchecked Sendable {
    
    private let requestIdentifier: String
    private let obvMessage: ObvMessage
    private let sentMessageVotedTo: ObvMessageAppIdentifier
    ...
    public init(requestIdentifier: String, obvMessage: ObvMessage, sentMessageVotedTo: ObvMessageAppIdentifier, reactor: ObvContactIdentifier, userNotificationCategory: ObvUserNotificationCategoryIdentifier, creator: PersistedUserNotification.Creator) {
        self.requestIdentifier = requestIdentifier
        self.obvMessage = obvMessage
        self.sentMessageVotedTo = sentMessageVotedTo
    ...
    public override func main(obvContext: ObvContext, viewContext: NSManagedObjectContext) {
        ...
        do {
            ...
            if try PersistedUserNotification.exists(requestIdentifier: requestIdentifier, obvMessage: obvMessage, within: obvContext.context) {
                ...
                _ = try PersistedUserNotification.createForPollVoteOnSentMessage(
                    creator: creator,
