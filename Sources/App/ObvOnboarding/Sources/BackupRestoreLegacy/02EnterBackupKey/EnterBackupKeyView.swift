...
struct EnterBackupKeyView: View, BackupKeyTextFieldActionsProtocol {
    ...
    struct Model {
        ...
        let initialValueOfBackupSeedManuallyEntered: BackupSeed? // Set if the user entered a seed during the new backup flow
    }
        ...
        VStack {
            ScrollView {
                VStack {
                    ...
                    ObvHeaderView(
                        title: "ONBOARDING_ENTER_BACKUP_KEY".localizedInThisBundle,
                        subtitle: nil)
                    .padding(.bottom, 35)
                    
                    BackupKeyTextField(model: .init(showClearButton: showClearButton,
                                                    acceptableCharactersForBackupKeyString: model.acceptableCharactersForBackupKeyString,
                                                    initialValueOfBackupSeedManuallyEntered: model.initialValueOfBackupSeedManuallyEntered),
                                       actions: self)
                    .padding(.horizontal)
...
private struct BackupKeyTextField: View, SingleTextFieldActions {
    ...
    struct Model {
        ...
        let acceptableCharactersForBackupKeyString: CharacterSet
        let initialValueOfBackupSeedManuallyEntered: BackupSeed?
    }
    ...
    private func onTask() {
        guard let initialValueOfBackupSeedManuallyEntered = model.initialValueOfBackupSeedManuallyEntered else { return }
        let description = initialValueOfBackupSeedManuallyEntered.description
        guard description.count == 32 else { return }
        _ = tryToPasteTextIfItIsSomeBackupKey(description)
    }
...
struct EnterBackupKeyView_Previews: PreviewProvider {
    ...
    private static let model = EnterBackupKeyView.Model(encryptedBackup: Data(), acceptableCharactersForBackupKeyString: CharacterSet(charactersIn: "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"), initialValueOfBackupSeedManuallyEntered: .init("00000000000000000000000000000000"))
