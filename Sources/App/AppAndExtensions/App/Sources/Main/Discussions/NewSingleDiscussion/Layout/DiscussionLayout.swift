...
final class DiscussionLayout: UICollectionViewLayout {
    ...
    private var cacheOfLayoutAttributesForItemAtIndexPath = [IndexPath: UICollectionViewLayoutAttributes]()
    
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: DiscussionLayout.self))
...
extension DiscussionLayout {
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for sanitizedAttribute in sanitizedAttributes {
            cacheOfLayoutAttributesForItemAtIndexPath[sanitizedAttribute.indexPath] = sanitizedAttribute
        }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        guard indexPath.section < cachedItemInfos.count else {
            self.cacheOfLayoutAttributesForItemAtIndexPath[indexPath] = nil
            return nil
        }
        guard indexPath.item < cachedItemInfos[indexPath.section].count else {
            self.cacheOfLayoutAttributesForItemAtIndexPath[indexPath] = nil
            return nil
        }
        ...
        self.cacheOfLayoutAttributesForItemAtIndexPath[indexPath] = attributes
...
extension DiscussionLayout {
    ...
    override func finalLayoutAttributesForDisappearingItem(at itemIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if let frame = indexPathsBeingDeleted.removeValue(forKey: itemIndexPath) {
        
            // This method was called as an item is indeed being deleted. We return the final attributes
            ...
            // This method seems to be called even when no items are being deleted. To avoid unexpected animations (such as flickering cells)
            // during a collection view refresh, we return the cached attributes for this item.
            // Previously, we were returning `nil`, which caused visible cell flickering on each refresh.
            
            return cacheOfLayoutAttributesForItemAtIndexPath[itemIndexPath]
...
extension DiscussionLayout {
    ...
    private func adjustRelativeOriginOfLayoutItemInfosInSection(_ section: Int, untilItem lastItem: Int) {
        ...
        guard firstItemToAdjust <= lastItem else { assertionFailure(); return }
...
extension UICollectionViewUpdateItem.Action {
