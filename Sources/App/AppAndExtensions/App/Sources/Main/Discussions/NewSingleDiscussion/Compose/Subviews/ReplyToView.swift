...
final class ReplyToView: UIView {
    ...
    private static let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: "ReplyToView")

    /// Implementing `UIViewWithThumbnailsForUTI`
    var imageForUTI = [String: UIImage]()

    weak var cacheDelegate: DiscussionCacheDelegate?
    weak var delegate: ReplyToViewDelegate?
    ...
    @objc private func xmarkButtonTapped() {
        ...
        Task { [weak self] in
            ...
            try? await delegate.userWantsToRemoveReplyToMessage(draftObjectID: draftObjectID)
        }
    }
}


protocol ReplyToViewDelegate: AnyObject {
    func userWantsToRemoveReplyToMessage(draftObjectID: TypeSafeManagedObjectID<PersistedDraft>) async throws
}
