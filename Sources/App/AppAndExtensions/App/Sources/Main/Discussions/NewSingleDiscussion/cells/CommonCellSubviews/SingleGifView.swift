...
final class SingleGifView: ViewForOlvidStack, ViewWithMaskedCorners, ViewWithExpirationIndicator, UIViewWithTappableStuff {
    ...
    private func animateGif(localRefreshId: UUID) {
        ...
        guard let imageSourceOptions = [kCGImageSourceTypeIdentifierHint: UTType.gif.identifier] as? CFDictionary,
              ...
              let gifProperties = cgImageSourceProperties[kCGImagePropertyGIFDictionary] as? Dictionary<CFString, Any> ?? cgImageSourceProperties[kCGImagePropertyWebPDictionary] as? Dictionary<CFString, Any>,
              let canvasPixelWidth = gifProperties[kCGImagePropertyGIFCanvasPixelWidth] as? NSNumber ?? gifProperties[kCGImagePropertyWebPCanvasPixelWidth] as? NSNumber,
              let canvasPixelHeight = gifProperties[kCGImagePropertyGIFCanvasPixelHeight] as? NSNumber ?? gifProperties[kCGImagePropertyWebPCanvasPixelHeight] as? NSNumber
        else {
            assertionFailure()
            return
        }
