...
extension SystemMessageCell {
    ...
    var viewForTargetedPreview: UIView {
        guard let contentView = contentView as? SystemMessageCellContentView else {
            assertionFailure();
            return contentView
        }
        if let firstPopableContent = contentView.mainStack.popableSubviews.first {
            return firstPopableContent
        } else {
            return contentView.mainStack
        }
    }
