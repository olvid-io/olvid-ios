...
final class NewComposeMessageView: UIView, UITextViewDelegate, ViewShowingHardLinks {
    ...
    var viewForShareLocationTip: UIView? {
        guard let locationButton, let plusCircleButton else { return nil }
        return locationButton.isHidden ? plusCircleButton : locationButton
    }
    ...
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: NewComposeMessageView.self))
    private static let logger = Logger(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: NewComposeMessageView.self))
    ...
    private func button(for action: NewComposeMessageViewSortableAction) -> UIButton? {
        switch action {
    ...
    private func button(for action: NewComposeMessageViewUnsortableAction) -> UIButton? {
        switch action {
    ...
    private func actionTitle(for action: NewComposeMessageViewSortableAction) -> String {
    ...
    private func actionTitle(for action: NewComposeMessageViewUnsortableAction) -> String {
        switch action {
        case .composeMessageSettings:
            return action.title
        }
    ...
    private func isActionAvailable(for action: NewComposeMessageViewSortableAction) -> Bool {
    ...
    private func isActionAvailable(for action: NewComposeMessageViewUnsortableAction) -> Bool {
        guard !draft.isDeleted else { return false }
        switch action {
        case .composeMessageSettings:
            return true
    ...
    private func uiAction(for action: NewComposeMessageViewSortableAction) -> UIAction? {
    ...
    private func uiAction(for action: NewComposeMessageViewUnsortableAction) -> UIAction? {
        let symbolConfiguration = UIImage.SymbolConfiguration(textStyle: .body)
        let image = UIImage(systemIcon: action.icon, withConfiguration: symbolConfiguration)
        guard isActionAvailable(for: action) else { return nil }
        let title = actionTitle(for: action)
    ...
    init(draft: PersistedDraft, viewShowingHardLinksDelegate: ViewShowingHardLinksDelegate, cacheDelegate: DiscussionCacheDelegate?, delegate: NewComposeMessageViewDelegate?, datasource: NewComposeMessageViewDatasource?, attachmentsCollectionViewControllerDelegate: any AttachmentsCollectionViewControllerDelegate) {
        ...
        self.attachmentsCollectionViewController = AttachmentsCollectionViewController(draftObjectID: draft.typedObjectID,
                                                                                       ...
                                                                                       attachmentsCollectionViewControllerDelegate: attachmentsCollectionViewControllerDelegate)
        ...
        self.replyToView.delegate = self
    ...
    private func observeNotifications() {
        notificationTokens.append (
            ObvMessengerSettingsNotifications.observePreferredComposeMessageViewActionsDidChange { [weak self] in
    ...
    private func setupInternalViews() {
        ...
        for action in NewComposeMessageViewUnsortableAction.defaultOrder.reversed() {
        ...
        for action in ObvMessengerSettings.Interface.preferredComposeMessageViewActionsOrder.reversed() {
            guard isActionAvailable(for: action) else { continue }
            switch action {
            case .oneTimeEphemeralMessage:
    ...
    private func hideOrShowButtonsForAvailableWidth(forceUpdate: Bool = false) {
        ...
        if let plusButton = multipleButtonsStackView.arrangedSubviews.first as? UIButton, !plusButton.isHidden {
            let reorderableElements: [UIMenuElement] = ObvMessengerSettings.Interface.preferredComposeMessageViewActionsOrder.compactMap({ action in
                guard let button = button(for: action) else { return nil }
                guard button.isHidden else { return nil }
                return uiAction(for: action)
            })
            let unreorderableElements: [UIMenuElement] = NewComposeMessageViewUnsortableAction.defaultOrder.compactMap({ action in
                guard let button = button(for: action) else { return nil }
                guard button.isHidden else { return nil }
                return uiAction(for: action)
            })
    ...
    private func continuouslySaveDraftText() {
        ...
        .sink(receiveValue: { [weak self] (body, mentions) in
            Task { [weak self] in
                guard let self else { return }
                try? await delegate?.userWantsToUpdateDraftBodyAndMentions(self, draftObjectID: draftObjectID, body: body.body, mentions: mentions)
            }
        })
        .store(in: &cancellables)
    }
...
extension NewComposeMessageView {
    ...
    private func shareLocationButtonTapped() async {
...
// MARK: - Implementing ReplyToViewDelegate

extension NewComposeMessageView: ReplyToViewDelegate {
    
    func userWantsToRemoveReplyToMessage(draftObjectID: ObvUICoreData.TypeSafeManagedObjectID<ObvUICoreData.PersistedDraft>) async throws {
...
// MARK: - Implementing DraftSettingsHostingViewControllerDelegate

extension NewComposeMessageView: DraftSettingsHostingViewControllerDelegate {
        
    func userWantsToUpdateDraftExpiration(_ draftSettingsHostingViewController: DraftSettingsHostingViewController, draftObjectID: ObvUICoreData.TypeSafeManagedObjectID<ObvUICoreData.PersistedDraft>, value: ObvUICoreData.PersistedDiscussionSharedConfigurationValue?) async throws {
