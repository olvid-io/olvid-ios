...
fileprivate final class SentMessageInfosViewStore: ObservableObject {
    ...
    init?(messageSent: PersistedMessageSent) {
        ...
        self.discussionKind = try? messageSent.discussion?.kind
    ...
    private func computeNumberOfNewMessagesBeforeSuppression(within context: NSManagedObjectContext) -> Int? {
        guard let messageSent = try? PersistedMessageSent.get(with: messageSentObjectID, within: context) as? PersistedMessageSent else { return nil }
        guard messageSent.wasSentOrCouldNotBeSentToOneOrMoreRecipients else { return nil }
        guard let discussion = messageSent.discussion else { assertionFailure(); return nil }
...
protocol SentMessageInfosViewModelProtocol: ObservableObject, SentMessageMetadatasSectionViewModelProtocol {
}

struct SentMessageInfosView<Model: SentMessageInfosViewModelProtocol>: View {
    
    @ObservedObject var model: Model
    @ObservedObject fileprivate var store: SentMessageInfosViewStore
    
    var body: some View {
        SentMessageInfosInnerView(model: model,
                                  ...
                                  discussionKind: store.discussionKind)
...
private struct SentMessageInfosInnerView<Model: SentMessageInfosViewModelProtocol>: View {
    ...
    var body: some View {
        List {
            ...
            SentMessageMetadatasSectionView(model: model,
...
private struct Legend: View {
    ...
    private func localizedStringForStatus(_ status: PersistedMessageSent.MessageStatus) -> LocalizedStringKey {
        status.getLocalizedStringKey(messageHasMoreThanOneRecipient: messageHasMoreThanOneRecipient)
    }
    
    private func symbolIconForStatus(_ status: PersistedMessageSent.MessageStatus) -> any SymbolIcon {
        status.getSymbolIcon(messageHasMoreThanOneRecipient: messageHasMoreThanOneRecipient)
    }
    
    var body: some View {
        Section(header: Text("STATUS_LEGEND_TITLE"), content: {
            if messageHasMoreThanOneRecipient {
                ObvLabel(localizedStringForStatus(.unprocessed), symbolIcon: symbolIconForStatus(.unprocessed))
                ObvLabel(localizedStringForStatus(.processing), symbolIcon: symbolIconForStatus(.processing))
                ObvLabel(localizedStringForStatus(.sent), symbolIcon: symbolIconForStatus(.sent))
                ObvLabel(localizedStringForStatus(.partiallyDeliveredNotRead), symbolIcon: symbolIconForStatus(.partiallyDeliveredNotRead))
                ObvLabel(localizedStringForStatus(.partiallyDeliveredAndPartiallyRead), symbolIcon: symbolIconForStatus(.partiallyDeliveredAndPartiallyRead))
                ObvLabel(localizedStringForStatus(.fullyDeliveredAndNotRead), symbolIcon: symbolIconForStatus(.fullyDeliveredAndNotRead))
                ObvLabel(localizedStringForStatus(.fullyDeliveredAndPartiallyRead), symbolIcon: symbolIconForStatus(.fullyDeliveredAndPartiallyRead))
                ObvLabel(localizedStringForStatus(.fullyDeliveredAndFullyRead), symbolIcon: symbolIconForStatus(.fullyDeliveredAndFullyRead))
                ObvLabel(localizedStringForStatus(.couldNotBeSentToOneOrMoreRecipients), symbolIcon: symbolIconForStatus(.couldNotBeSentToOneOrMoreRecipients))
            } else {
                ObvLabel(localizedStringForStatus(.unprocessed), symbolIcon: symbolIconForStatus(.unprocessed))
                ObvLabel(localizedStringForStatus(.processing), symbolIcon: symbolIconForStatus(.processing))
                ObvLabel(localizedStringForStatus(.sent), symbolIcon: symbolIconForStatus(.sent))
...
extension PersistedMessageSent: SentMessageInfosViewModelProtocol {}
