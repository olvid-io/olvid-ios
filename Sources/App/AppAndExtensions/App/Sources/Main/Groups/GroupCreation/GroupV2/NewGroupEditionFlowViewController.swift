...
final class NewGroupEditionFlowViewController: UIViewController {
    ...
    private lazy var flowNavigationController: UINavigationController = {
        ...
        let mode: UsersSelectionForGroupViewController.Mode
        switch editionType {
        case .cloneGroup, .createGroup:
            mode = .create
        case .modifyGroup(delegate: _, groupIdentifier: let groupIdentifier):
            mode = .modify(groupIdentifier: groupIdentifier)
        }
        let rootViewController = UsersSelectionForGroupViewController(
            ownedCryptoId: ownedCryptoId,
            mode: mode,
            preSelectedUsers: groupProxyModel.selectedUsers,
            delegate: self)
    ...
    private func goTo(state: GroupCreationFlowState, animated: Bool = true) {
        ...
        case .selectType:
            if let selectTypeVC = flowNavigationController.viewControllers.first(where: { $0 is GroupCreationTypeHostingViewController }) {
                ...
                let vc = GroupCreationTypeHostingViewController(preselectedGroupType: groupProxyModel.groupType?.value,
                                                                selectedUsersOrdered: groupProxyModel.selectedUsers.sorted(),
                                                                delegate: self)
        ...
        case .advancedParameters:
            if let vc = flowNavigationController.viewControllers.first(where: { $0 is GroupCreationParametersHostingViewController }) {
                ...
                let vc = GroupCreationParametersHostingViewController(model: .init(selectedUsersOrdered: groupProxyModel.selectedUsers.sorted(),
        ...
        case .informations:
            if let infoVC = flowNavigationController.viewControllers.first(where: { $0 is GroupCreationInfoHostingViewController }) {
                ...
                let model = GroupInfoViewModel(selectedUsersOrdered: groupProxyModel.selectedUsers.sorted(),
        ...
        case .adminChoice(showButton: let showButton):
            ...
            if let vc = flowNavigationController.viewControllers.first(where: { $0 is GroupCreationAdminChoiceHostingViewController }) {
                ...
                let vc = GroupCreationAdminChoiceHostingViewController(users: groupProxyModel.selectedUsers.sorted(),
...
extension NewGroupEditionFlowViewController: GroupCreationAdminChoiceHostingViewControllerDelegate {
    ...
    func userWantsToChangeUserAdminStatus(in controller: GroupCreationAdminChoiceHostingViewController, userCryptoId: ObvTypes.ObvCryptoId, isAdmin: Bool) -> Set<ObvUICoreData.PersistedUser> {
        groupProxyModel.changeUserAdminStatus(userCryptoId: userCryptoId, isAdmin: isAdmin)
...
// MARK: - UsersSelectionForGroupViewControllerDelegate

extension NewGroupEditionFlowViewController: UsersSelectionForGroupViewControllerDelegate {
            
    func userDidValidateSelectedUsers(in controller: UsersSelectionForGroupViewController, selectedUsers: [ObvUICoreData.PersistedUser]) async {
        groupProxyModel.setSelectedUsers(to: Set(selectedUsers))
        goTo(state: .selectType)
    }
    
    func userWantsToCancelGroupCreationFlow(in controller: UsersSelectionForGroupViewController) {
        userWantsToCancelGroupCreationFlow()
    }
...
extension NewGroupEditionFlowViewController: GroupCreationTypeHostingViewControllerDelegate {
    ...
    func userDidSelectGroupType(in controller: GroupCreationTypeHostingViewController, selectedGroupType: GroupTypeValue) async {
        ...
        case .managed, .readOnly:
            if groupProxyModel.selectedUsers.isEmpty {
                goTo(state: .informations)
            } else {
...
extension NewGroupEditionFlowViewController {
    ...
    private func startCreationFlow(delegate: NewGroupEditionFlowViewControllerGroupCreationDelegate) async {
        ...
        let otherGroupMembers: [ObvGroupV2.IdentityAndPermissions] = groupProxyModel.selectedUsers
            .map { contact in
    ...
    private func startUpdateFlow(delegate: NewGroupEditionFlowViewControllerGroupModificationDelegate, groupIdentifier: Data) async throws {
        ...
        let cryptoIdsOfSelectedMembers = Set(groupProxyModel.selectedUsers.map(\.cryptoId))
