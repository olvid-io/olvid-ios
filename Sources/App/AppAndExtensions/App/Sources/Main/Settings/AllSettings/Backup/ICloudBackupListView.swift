...
protocol ICloudBackupListViewControllerDelegate: AnyObject {
    @MainActor func lastCloudBackupForCurrentDeviceWasDeleted()
}
...
fileprivate final class ICloudBackupListViewModel: ObservableObject, @unchecked Sendable {
        ...
        Task {
            do {
                ...
                if isLastForCurrentDevice {
                    await self.delegate?.lastCloudBackupForCurrentDeviceWasDeleted()
                }
            } catch(let error) {
                DispatchQueue.main.async {
                    withAnimation {
                        ...
                        cell
                            .swipeActions {
                                Button(role: .destructive) {
                                    ...
                                    Label(CommonString.Word.Delete, systemImage: SystemIcon.trash.name)
                                }
                            }
                    }
