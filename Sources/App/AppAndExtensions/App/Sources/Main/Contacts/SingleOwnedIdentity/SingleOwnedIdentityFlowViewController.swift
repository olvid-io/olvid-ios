...
protocol SingleOwnedIdentityFlowViewControllerDelegate: AnyObject, StoreKitDelegate {
    ...
    func userWantsToUnbindOwnedIdentityFromKeycloak(_ viewController: SingleOwnedIdentityFlowViewController, ownedCryptoId: ObvCryptoId) async throws(ObvTypes.ObvUnbindOwnedIdentityFromKeycloakError)
}
...
final class SingleOwnedIdentityFlowViewController: UIHostingController<SingleOwnedIdentityView>, HiddenProfilePasswordChooserViewControllerDelegate, OwnedIdentityDetailedInfosViewDelegate, SingleOwnedIdentityViewActionsDelegate, OwnedDevicesListViewActionsDelegate, PermuteDeviceExpirationHostingViewControllerDelegate, ChooseDeviceToReactivateHostingViewControllerDelegate {
    ...
    private static let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: "SingleOwnedIdentityFlowViewController")
    ...
    private func userWantsToUnbindFromKeycloakServer(ownedCryptoId: ObvCryptoId) async {
        ...
        do {
            try await delegate.userWantsToUnbindOwnedIdentityFromKeycloak(self, ownedCryptoId: ownedCryptoId)
            showHUD(type: .checkmark)
            try? await Task.sleep(seconds: 2)
            hideHUD()
        } catch {
            ...
            case .userCannotUnbindAsTransferIsRestricted:
                hideHUD()
                let title = String(localized: "CANNOT_MANUALLY_UNBIND_AS_TRANSFER_IS_RESTRICTED_TITLE")
                let message = String(localized: "CANNOT_MANUALLY_UNBIND_AS_TRANSFER_IS_RESTRICTED_MESSAGE")
                let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
                alert.addAction(.init(title: CommonString.Word.Ok, style: .default))
...
extension SingleOwnedIdentityFlowViewController {
    ...
    func getIsTransferRestricted(ownedCryptoId: ObvCryptoId) async throws -> Bool {
        let (obvKeycloakState, _) = try obvEngine.getOwnedIdentityKeycloakState(with: ownedCryptoId)
        return obvKeycloakState?.isTransferRestricted ?? false
    }
...
extension SingleOwnedIdentityFlowViewController {
    ...
    func userWantsToReactivateThisDevice(ownedCryptoId: ObvCryptoId) async {
        ...
        guard let currentDeviceObj = ownedIdentity.currentDevice else { assertionFailure(); return }
