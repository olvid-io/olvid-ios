...
/// Is implemented by ``PersistedInvitation``
    ...
    var body: some View {
        VStack {
            ...
            if !model.groupMembers.isEmpty {
                VStack(alignment: .leading) {
                    HStack {
                        Text("\(UInt(model.groupMembers.count))_GROUP_MEMBERS")
                            .font(.subheadline)
                        Spacer()
                    }
...
//                        } else {
  ...
  //                      }
                    ...
                    case .plainForAbortingProtocol(obvDialog: let obvDialog, localizedTitle: let localizedTitle):
                        Button(role: .destructive, action: { isAbortConfirmationShown = true }, label: { Text(verbatim: localizedTitle) })
...
private struct SASTextField: View, SingleSASDigitTextFielddActions {
    ...
    private var showClearButton: Bool {
        switch model.mode {
        case .enterSAS:
            return textValues.first(where: { !$0.isEmpty }) != nil
                ...
                case .enterSAS:
                    ...
                    SingleSASDigitTextField("X", text: $textValue3, actions: self, model: .init(index: 3))
                        ...
                        .overlay(alignment: .center) {
                            ...
                            .offset(x: SingleSASDigitTextField.rectangleSize.width + 4, y: 0)
...
struct SingleSASDigitTextField: View {
    ...
    fileprivate static let rectangleSize = CGSize(width: 43, height: 48)
    private static let rectangleCornerRadius: CGFloat = 14
    ...
    var body: some View {
        TextField(key, text: text)
            ...
            .overlay(content: {
                RoundedRectangle(cornerRadius: Self.rectangleCornerRadius, style: .continuous)
                    .stroke(Color(UIColor.systemGray2), lineWidth: 1)
                    .frame(width: Self.rectangleSize.width, height: Self.rectangleSize.height)
                    .padding(.horizontal, 1)
            })
            .background(
                RoundedRectangle(cornerRadius: Self.rectangleCornerRadius)
