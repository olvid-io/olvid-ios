...
final class ThumbnailManager {
    ...
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: ThumbnailManager.self))
    ...
    private func observeUserIsLeavingDiscussionNotifications() {
        observationTokens.append(ObvMessengerInternalNotification.observeCurrentDiscussionDidChange { [weak self] previousDiscussion, currentDiscussion in
            guard previousDiscussion != currentDiscussion else { return }
