...
final actor AppBackupManager: AppBackupDelegate, ObvErrorMaker {
    ...
    private static let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: AppBackupManager.self))
...
extension AppBackupManager {
    ...
    private func uploadBackupFileToCloudKit(backupFile: BackupFile, container: CKContainer, backupRequestUuid: UUID, manuallyRequestByUser: Bool) async throws {
        ...
        let record = CKRecord(recordType: ObvAppCoreConstants.BackupConstants.recordType, recordID: recordID)
...
fileprivate final class BackupFile: UIActivityItemProvider, @unchecked Sendable {
...
extension CKRecord {
    ...
    subscript(key: ObvAppCoreConstants.BackupConstants.Key) -> CKRecordValue? {
        get {
            self[key.rawValue]
        }
        set {
