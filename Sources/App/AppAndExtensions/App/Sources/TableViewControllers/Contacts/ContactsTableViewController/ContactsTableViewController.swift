...
class ContactsTableViewController: UITableViewController {
    ...
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: ContactsTableViewController.self))
...
extension ContactsTableViewController {
    ...
    private func configure(_ cell: ObvSubtitleTableViewCell, withObjectAtIndexPath frcIndexPath: IndexPath) {
        guard let contact = fetchedResultsController.safeObject(at: frcIndexPath) else { return }
        configure(cell, with: contact)
    }
...
extension ContactsTableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") { [weak self] (action, view, handler) in
            guard let contact = self?.fetchedResultsController.safeObject(at: frcIndexPath) else { return }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let frcIndexPath: IndexPath
        frcIndexPath = indexPath
        guard let persistedContactIdentity = fetchedResultsController.safeObject(at: frcIndexPath) else { return }
