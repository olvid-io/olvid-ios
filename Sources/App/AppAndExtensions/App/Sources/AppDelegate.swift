...
class AppDelegate: UIResponder, UIApplicationDelegate, ObvErrorMaker {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // Initialize the UserNotificationsCoordinator as it registers the UNUserNotificationCenter delegate.
    ...
    /// This is also called while in the foreground. Tests show that this is called even after the app is force-quit.
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable: Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        os_log("🫸🌊 Application did receive remote notification", log: log, type: .info)
        // Note about the warning: it seems there is nothing we can do.
        // Even the async version of this delegate method shows a warning as soon as we declare it.
