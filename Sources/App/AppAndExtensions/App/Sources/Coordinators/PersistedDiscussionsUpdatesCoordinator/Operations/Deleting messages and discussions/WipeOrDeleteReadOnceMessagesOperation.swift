...
final class WipeOrDeleteReadOnceMessagesOperation: ContextualOperationWithSpecificReasonForCancel<CoreDataOperationReasonForCancel>, @unchecked Sendable {
    
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: WipeOrDeleteReadOnceMessagesOperation.self))
    ...
    override func main(obvContext: ObvContext, viewContext: NSManagedObjectContext) {
        ...
        for sentMessage in sentMessages {
            do {
                let info = try sentMessage.wipeOrDeleteExpiredMessageSent()
                infos += [info]
            } catch {
        ...
        if !preserveReceivedMessages {
            ...
            for receivedMessage in receivedMessages {
                do {
                    let info = try receivedMessage.deleteExpiredMessage()
                    infos += [info]
                } catch {
