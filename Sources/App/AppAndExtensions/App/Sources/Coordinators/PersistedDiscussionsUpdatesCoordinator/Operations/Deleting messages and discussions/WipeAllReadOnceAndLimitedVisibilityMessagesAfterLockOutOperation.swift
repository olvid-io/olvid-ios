...
protocol WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperationDelegate: AnyObject {
...
final class WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperation: ContextualOperationWithSpecificReasonForCancel<CoreDataOperationReasonForCancel>, @unchecked Sendable {
    
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperation.self))
    ...
    private let appType: ObvUICoreDataConstants.AppCategory
    private let wipeType: WipeType
    private weak var delegate: WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperationDelegate?
    ...
    init(userDefaults: UserDefaults?, appType: ObvUICoreDataConstants.AppCategory, wipeType: WipeType, delegate: WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperationDelegate?) {
        ...
        do {
            ...
            try obvContext.addContextDidSaveCompletionHandler { error in
                ...
                DispatchQueue(label: "Queue created in WipeAllReadOnceAndLimitedVisibilityMessagesAfterLockOutOperation").async {
                    if earlyAbortWipe {
                        localDelegate?.setExtensionFailedToWipeAllEphemeralMessagesBeforeDateOnUserDefaults(timestampOfLastMessageToWipe: timestampOfLastMessageToWipe)
                    } else {
                        localDelegate?.setExtensionFailedToWipeAllEphemeralMessagesBeforeDateOnUserDefaults(timestampOfLastMessageToWipe: nil)
                    }
                }
            }
...
fileprivate extension ObvUICoreDataConstants.AppCategory {
