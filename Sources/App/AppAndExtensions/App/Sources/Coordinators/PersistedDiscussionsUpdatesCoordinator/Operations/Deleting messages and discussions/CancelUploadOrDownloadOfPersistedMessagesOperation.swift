...
final class DetermineEngineIdentifiersOfMessagesToCancelOperation: ContextualOperationWithSpecificReasonForCancel<DetermineEngineIdentifiersOfMessagesToCancelOperation.ReasonForCancel>, @unchecked Sendable, OperationProvidingListOfMessagesToCancelFromEngine {
    
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: CancelUploadOrDownloadOfPersistedMessagesOperation.self))
        ...
        do {
            ...
            if let error = error as? ObvUICoreDataError {
                ...
                case .couldNotFindOneToOneContactWithId(contactIdentifier: _):
                    // No assert in this case, we might be deleting an old locked one2one discussion
                    return
                default:
                    assertionFailure()
...
final class CancelUploadOrDownloadOfPersistedMessagesOperation: AsyncOperationWithSpecificReasonForCancel<CancelUploadOrDownloadOfPersistedMessagesOperation.ReasonForCancel>, @unchecked Sendable {
