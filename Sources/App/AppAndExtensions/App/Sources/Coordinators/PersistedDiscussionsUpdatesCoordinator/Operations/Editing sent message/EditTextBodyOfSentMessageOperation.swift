...
final class EditTextBodyOfSentMessageOperation: ContextualOperationWithSpecificReasonForCancel<EditTextBodyOfSentMessageOperation.ReasonForCancel>, @unchecked Sendable {
    ...
    private(set) var updateMessageJSONToSend: UpdateMessageJSONToSend? // Set if there is something to send to the engine
    ...
    override func main(obvContext: ObvContext, viewContext: NSManagedObjectContext) {
        ...
        do {
            ...
            if let updatedMessage {
                ...
                // If we reach this point, we certainly have to construct the input of the operation in charge of sending the message
                
                if obvContext.context.hasChanges {
                    
                    self.updateMessageJSONToSend = try updatedMessage.toUpdateMessageJSONToSend()
