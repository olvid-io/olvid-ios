...
enum ObvMessengerInternalNotification {
	...
	case userWantsToBindOwnedIdentityToKeycloak(ownedCryptoId: ObvCryptoId, obvKeycloakState: ObvKeycloakState, keycloakUserId: String, completionHandler: (Error?) -> Void)
	...
	static func observeUserWantsToBindOwnedIdentityToKeycloak(object obj: Any? = nil, queue: OperationQueue? = nil, block: @escaping (ObvCryptoId, ObvKeycloakState, String, @escaping (Error?) -> Void) -> Void) -> NSObjectProtocol {
		...
		return NotificationCenter.default.addObserver(forName: name, object: obj, queue: queue) { (notification) in
			...
			let completionHandler = notification.userInfo!["completionHandler"] as! (Error?) -> Void
