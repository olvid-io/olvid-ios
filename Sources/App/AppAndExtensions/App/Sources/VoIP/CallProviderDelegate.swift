...
final class CallProviderDelegate: NSObject {
    ...
    weak var signalingDelegate: CallProviderDelegateSignalingDelegate?
    ...
    init(obvEngine: ObvEngine) {
        ...
        Task { [weak self] in
            ...
            await self.callManager.setNCXCallControllerDelegate(self.callProviderHolder.ncxCallControllerDelegate)
...
extension CallProviderDelegate: PKPushRegistryDelegate {
    ...
    func pushRegistry(_ registry: PKPushRegistry, didReceiveIncomingPushWith payload: PKPushPayload, for type: PKPushType, completion: @escaping () -> Void) {
        ...
        guard let encryptedNotification = ObvEncryptedRemoteUserNotification(dict: payload.dictionaryPayload) else {
        ...
        callProviderHolder.provider.reportNewIncomingCall(with: callIdentifierForCallKit, update: initalUpdate) { [weak self] error in
            ...
            if let error {
                switch error {
                case let error as CXErrorCodeIncomingCallError:
                    switch error.code {
                    case .filteredByDoNotDisturb:
                        os_log("☎️✅❌ We failed to report an incoming call as the called was filtered by DoNotDisturb: %{public}@", log: Self.log, type: .error, error.localizedDescription)
                        Task { [weak self] in await self?.reportCallFilteredByDoNotDisturb(encryptedNotification: encryptedNotification, callIdentifierForCallKit: callIdentifierForCallKit) }
                ...
                os_log("☎️✅❌ We failed to report an incoming call: %{public}@", log: Self.log, type: .fault, error.localizedDescription)
                DispatchQueue.main.async {
                    completion()
                }
                assertionFailure()
    ...
    private func reportCallFilteredByDoNotDisturb(encryptedNotification: ObvEncryptedRemoteUserNotification, callIdentifierForCallKit: UUID) async {
        
        // Wait for the (decrypted) start call message
        
        let callerDeviceIdentifier: ObvContactDeviceIdentifier
        let startCallMessage: StartCallMessageJSON
        let uuidForWebRTC: UUID
        do {
            (callerDeviceIdentifier, startCallMessage, uuidForWebRTC) = try await pushKitNotificationSynchronizer.waitForStartCallMessage(encryptedNotification: encryptedNotification)
        } catch {
        ...
        guard let caller = await getOlvidCallParticipantInfo(contactIdentifier: callerDeviceIdentifier.contactIdentifier, isCaller: true) else { assertionFailure(); return }

        VoIPNotification.reportCallEvent(callUUID: encryptedNotification.messageIdFromServer.deterministicUUID,
                                         callReport: .filteredIncomingCall(caller: caller, initialOtherParticipantsCount: startCallMessage.participantCount),
                                         groupId: nil,
        ...
        do {
            if try await ownedIdentityHasSeveralDevices(ownedCryptoId: callerDeviceIdentifier.ownedCryptoId) {
                return
            } else {
                ...
                await newWebRTCMessageToSendToSingleContactDevice(webrtcMessage: rejectedMessage, contactDeviceIdentifier: callerDeviceIdentifier)
            }
        } catch {
    ...
    private func didReportNewIncomingCallToCallKit(encryptedNotification: ObvEncryptedRemoteUserNotification, callIdentifierForCallKit: UUID) async {
    ...
    private func processReceivedWebRTCMessage(messageType: WebRTCMessageJSON.MessageType, serializedMessagePayload: String, uuidForWebRTC: UUID, fromOlvidUser: OlvidUserId, messageIdentifierFromEngine: UID?) async {
        ...
        do {
            ...
            case .startCall:
                ...
                guard let callerDeviceIdentifier = fromOlvidUser.contactDeviceIdentifier else { assertionFailure(); return }
                guard ObvMessengerSettings.VoIP.receiveCallsOnThisDevice else {
                    ...
                    if try await ownedIdentityHasSeveralDevices(ownedCryptoId: fromOlvidUser.ownCryptoId) {
                        ...
                        await newWebRTCMessageToSendToAllContactDevices(webrtcMessage: rejectedMessage, contactIdentifier: contactIdentifier, forStartingCall: false)
                        // Notify the local user that a call was missed
                        let caller = OlvidCallParticipantInfo(contactObjectID: contactID, contactIdentifier: contactIdentifier, isCaller: true)
                ...
                if ObvUICoreDataConstants.useCallKit {
                    await pushKitNotificationSynchronizer.continuePushKitNotificationProcessing(startCallMessage, messageIdFromServer: messageIdentifierFromEngine, callerDeviceIdentifier: callerDeviceIdentifier, uuidForWebRTC: uuidForWebRTC)
                } else {
            ...
            case .answerCall:
                // This is one of the 3 message types that allow to set the call participant device UID (this is done in OlvidCall)
...
extension CallProviderDelegate: OlvidCallDelegate {
    ...
    func newWebRTCMessageToSendToAllContactDevices(webrtcMessage: ObvUICoreData.WebRTCMessageJSON, contactIdentifier: ObvContactIdentifier, forStartingCall: Bool) async {
        ...
        Task {
            await signalingDelegate.newWebRTCMessageToSendToAllContactDevices(webrtcMessage: webrtcMessage, contactIdentifier: contactIdentifier, forStartingCall: forStartingCall)
        }
    }
    ...
    func newWebRTCMessageToSendToSingleContactDevice(webrtcMessage: WebRTCMessageJSON, contactDeviceIdentifier: ObvContactDeviceIdentifier) async {
        ...
        guard let signalingDelegate else { assertionFailure(); return }
        Task {
            await signalingDelegate.newWebRTCMessageToSendToSingleContactDevice(webrtcMessage: webrtcMessage, contactDeviceIdentifier: contactDeviceIdentifier)
        }
    }
    ...
    private static func report(call: OlvidCall, report: CallReport) {
        ...
        debugPrint(report.participantInfos)
...
fileprivate extension CallProviderDelegate {
    ...
    @MainActor
    func getOlvidCallParticipantInfo(contactIdentifier: ObvContactIdentifier, isCaller: Bool) -> OlvidCallParticipantInfo? {
        guard let contact = try? PersistedObvContactIdentity.get(persisted: contactIdentifier, whereOneToOneStatusIs: .any, within: ObvStack.shared.viewContext) else { return nil }
        return .init(contactObjectID: contact.typedObjectID, contactIdentifier: contactIdentifier, isCaller: isCaller)
    }
...
protocol CallProviderDelegateSignalingDelegate: AnyObject {
    
    func newWebRTCMessageToSendToAllContactDevices(webrtcMessage: WebRTCMessageJSON, contactIdentifier: ObvContactIdentifier, forStartingCall: Bool) async
    func newWebRTCMessageToSendToSingleContactDevice(webrtcMessage: WebRTCMessageJSON, contactDeviceIdentifier: ObvContactDeviceIdentifier) async
