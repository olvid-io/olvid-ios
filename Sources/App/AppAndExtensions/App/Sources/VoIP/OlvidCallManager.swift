...
actor OlvidCallManager {
    ...
    func createIncomingCall(uuidForCallKit: UUID, uuidForWebRTC: UUID, callerDeviceIdentifier: ObvContactDeviceIdentifier, startCallMessage: StartCallMessageJSON, rtcPeerConnectionQueue: OperationQueue, callDelegate: OlvidCallDelegate) async throws -> OlvidCall {
...
extension OlvidCallManager {
    ...
    func processHangedUpMessage(_ hangedUpMessage: HangedUpMessageJSON, uuidForWebRTC: UUID, contact: OlvidUserId) async throws -> (call: OlvidCall, report: CallReport?) {
        ...
        let initialOtherParticipantsCount = call.initialParticipantCount

        let callReport: CallReport?
        if callStateWasInitial && call.direction == .incoming {
            callReport = .missedIncomingCall(caller: participantInfo, initialOtherParticipantsCount: initialOtherParticipantsCount)
    ...
    func processAnsweredOrRejectedOnOtherDeviceMessage(answered: Bool, uuidForWebRTC: UUID, ownedCryptoId: ObvCryptoId) async throws -> (incomingCall: OlvidCall?, callReport: CallReport?, cxCallEndedReason: CXCallEndedReason?) {
        ...
        if let incomingCall = callWithCallIdentifierForWebRTC(uuidForWebRTC) {
            ...
            guard incomingCall.direction == .incoming else {
                // This situation occurs exceptionally when the recipient's profile is also present on the caller's device.
                // In that situation, we discard the received message.
                return (nil, nil, nil)
            }
