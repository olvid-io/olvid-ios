...
public final class PersistedObvContactDevice: NSManagedObject, Identifiable {
    ...
    @NSManaged private var location: PersistedLocationContinuousReceived? // Non-nil when the contact is currently sharing her location from this device
    ...
    func updateWith(obvContactDevice device: ObvContactDevice) throws {
        guard try self.identity?.obvContactIdentifier == device.contactIdentifier, self.identifier == device.identifier else {
            assertionFailure()
            throw ObvUICoreDataError.unexpectedContactDeviceIdentifier
        }
