...
public struct NewSubscriptionStatusView: View {
    
    let title: Text?
    let apiKeyStatus: APIKeyStatus
    let apiKeyExpirationDate: Date?
    let userWantsToSeeSubscriptionPlansAction: (() -> Void)?
    let refreshStatusAction: (() -> Void)?
    let apiPermissions: APIPermissions
    let activateLicenseButtonAction: (() -> Void)?
    @Binding var isAPIKeyActivating: Bool
    let transitionID: String
    
    public init(title: Text?, apiKeyStatus: APIKeyStatus, apiKeyExpirationDate: Date?, userWantsToSeeSubscriptionPlansAction: (() -> Void)? = nil, refreshStatusAction:(() -> Void)? = nil, apiPermissions: APIPermissions, activateLicenseButtonAction: (() -> Void)? = nil, isAPIKeyActivating: Binding<Bool> = .constant(false), transitionID: String = "") {
        self.title = title
        self.apiKeyStatus = apiKeyStatus
        self.apiKeyExpirationDate = apiKeyExpirationDate
        self.userWantsToSeeSubscriptionPlansAction = userWantsToSeeSubscriptionPlansAction
        self.refreshStatusAction = refreshStatusAction
        self.apiPermissions = apiPermissions
        self.activateLicenseButtonAction = activateLicenseButtonAction
        self._isAPIKeyActivating = isAPIKeyActivating
    ...
    private let freeFeature: [FeatureView.Model] = [
            ...
            ObvCardView {
                VStack(alignment: .leading, spacing: 0) {
                    SubscriptionStatusSummaryView(apiKeyStatus: apiKeyStatus,
                        ...
                        .padding(.bottom, 16)
                    if let userWantsToSeeSubscriptionPlansAction {
                        OlvidButtonNew(action: userWantsToSeeSubscriptionPlansAction) {
                            Label(title: { Text("See subscription plans") }, icon: { Image(systemIcon: .storefront) })
                        }
                        .padding(.bottom, 16)
                    }
                    if let activateLicenseButtonAction {
                        ActivateLicenseButton(action: activateLicenseButtonAction, isAPIKeyActivating: $isAPIKeyActivating)
                            .padding(.bottom, 16)
                    }
                    ...
                    if let refreshStatusAction {
                        RefreshStatusButton(action: refreshStatusAction)
                            .padding(.bottom, 16)
                    }
                }
            }
...
// MARK: - See subscription plans button

private struct SeeSubscriptionPlansButton: View {
    
    let action: () -> Void
    
    private let systemIcon: SystemIcon = .flameFill
    
    var body: some View {
        if #available(iOS 26.0, *) {
            Button(action: action) {
                HStack {
                    Label(title: { Text("See subscription plans") }, icon: { Image(systemIcon: systemIcon) })
                }
                .padding(.vertical, 8)
            }
...
// MARK: - Internal view: RefreshStatusButton

private struct RefreshStatusButton: View {
    
    let action: () -> Void
    
    private let systemIcon: SystemIcon = .arrowClockwise
    
    var body: some View {
        if #available(iOS 26.0, *) {
            Button(action: action) {
                HStack {
                    Label(title: { Text("Refresh status") }, icon: { Image(systemIcon: systemIcon) })
                }
                .padding(.vertical, 8)
            }
            .buttonSizing(.flexible)
...
// MARK: - Internal view: ActivateLicenseButton

private struct ActivateLicenseButton: View {
    
    let action: () -> Void
    @Binding var isAPIKeyActivating: Bool
    
    private let systemIcon: SystemIcon = .checkmarkSealFill
    
    var body: some View {
        if #available(iOS 26.0, *) {
            Button(action: action) {
                HStack {
                    if isAPIKeyActivating { ProgressView().tint(.white) }
                    Label(title: { Text("ACTIVATE_NEW_LICENCE") }, icon: { Image(systemIcon: systemIcon) })
                }
                .padding(.vertical, 8)
            }
            .buttonSizing(.flexible)
...
struct FeatureListView_Previews: PreviewProvider {
    ...
    private static let testFreeFeatures = [
        NewSubscriptionStatusView.Feature(imageSystemName: "bubble.left.and.bubble.right.fill",
                                       imageColor: Color(.displayP3, red: 1.0, green: 0.35, blue: 0.39, opacity: 1.0),
                                       description: "Send & receive messages and attachments"),
        NewSubscriptionStatusView.Feature(imageSystemName: "person.3.fill",
                                       imageColor: Color(.displayP3, red: 7.0/255, green: 132.0/255, blue: 254.0/255, opacity: 1.0),
                                       description: "Create groups"),
        NewSubscriptionStatusView.Feature(imageSystemName: "phone.fill.arrow.down.left",
    ...
    private static let testPremiumFeatures = [
        NewSubscriptionStatusView.Feature(imageSystemName: "phone.fill.arrow.up.right",
                                       imageColor: Color(.displayP3, red: 253.0/255, green: 56.0/255, blue: 95.0/255, opacity: 1.0),
