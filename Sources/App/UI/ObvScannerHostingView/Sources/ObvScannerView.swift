...
public protocol ObvScannerHostingViewDelegate: AnyObject {
    func scannerViewActionButtonWasTapped() async
    func qrCodeWasScanned(olvidURL: OlvidURL) async
}

/// This scanner is still used in the onboarding. The scanner used in the invitation is part of the `ObvInvitationFlow` module.
public final class ObvScannerHostingView: UIHostingController<ScannerView>, ScannerViewStoreDelegate {
    
    let store: ScannerViewStore
    
    weak var delegate: ObvScannerHostingViewDelegate?
    
    public init(buttonType: ScannerView.ButtonType, delegate: ObvScannerHostingViewDelegate) {
        let store = ScannerViewStore()
    ...
    public func qrCodeWasScanned(olvidURL: OlvidURL) {
        Task { [weak self] in
...
public protocol ScannerViewStoreDelegate: AnyObject {
    func buttonAction()
...
public struct ScannerView: View {
    ...
    public init(buttonType: ButtonType, buttonAction: @escaping () -> Void, qrCodeScannedAction: @escaping (OlvidURL) -> Void) {
...
fileprivate final class NewQRCodeScannerViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: NewQRCodeScannerViewController.self))
