...
extension ImagePickerCoordinator: PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        itemProvider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
            ...
            Task {
                await self?.parent.setImage(to: image)
                await self?.parent.callCompletionHandler()
            }
        }
    }
...
struct ImagePicker: UIViewControllerRepresentable {
    ...
    @MainActor
    fileprivate func setImage(to newImage: UIImage?) {
        self.image = newImage
    }
    
    @MainActor
    fileprivate func callCompletionHandler() {
        completionHandler?()
    }
