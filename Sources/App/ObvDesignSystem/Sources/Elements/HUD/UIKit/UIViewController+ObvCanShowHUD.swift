...
extension UIViewController: ObvCanShowHUD {
    ...
    public func showHUDAndAwaitAnimationEnd(type: ObvHUDType) async {
    ...
    public func showThenHideHUD(type: ObvHUDType, andHideAfter timeInterval: TimeInterval = 1.5) async {
        await showHUDAndAwaitAnimationEnd(type: type)
        try? await Task.sleep(seconds: timeInterval)
        hideHUD()
    }
    

    public func showHUD(type: ObvHUDType, completionHandler: (() -> Void)? = nil) {
