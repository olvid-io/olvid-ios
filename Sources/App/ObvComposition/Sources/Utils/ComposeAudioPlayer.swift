...
@MainActor
final class ComposeAudioPlayer: NSObject, @MainActor AVAudioPlayerDelegate, ObservableObject {
    ...
    public static let shared: ComposeAudioPlayer = ComposeAudioPlayer()
    
    private let logger = Logger(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: ComposeAudioPlayer.self))
    ...
    weak var delegate: ObvAudioPlayerDelegate? {
        willSet { // When delegate changed, we stop previous audioURL attached to this delegate to play.
            delegate?.playerWillChangeCurrentFyle(previousAudioURL: currentURL)
        }
    }
    ...
    func play(_ audioURL: URL, at time: TimeInterval? = 0) -> Bool {
        currentURL = audioURL
        do {
            ...
            timer = Timer(timeInterval: 0.03, repeats: true) { [weak self] _ in
                Task { @MainActor in
                    ...
                    if let currentTime = self.audioPlayer?.currentTime {
                        self.delegate?.audioIsPlaying(currentTime: currentTime)
                    }
            ...
            logger.info("🎵 Start playing \(audioURL.lastPathComponent, privacy: .public) with speaker \(shouldUseSpeaker ? "enabled" : "disabled", privacy: .public)")
