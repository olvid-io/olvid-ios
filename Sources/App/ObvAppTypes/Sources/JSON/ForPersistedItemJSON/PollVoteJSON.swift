...
public struct PollVoteJSON: Codable, Sendable {
    
    private let log = OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: "PollVoteJSON")
    ...
    public func makeError(message: String) -> Error { PollVoteJSON.makeError(message: message) }
    ...
    /// When sending a `PollVoteJSON`, we set `originalServerTimestamp` when re-sending a poll vote to a member who switched from pending to non-pending.
