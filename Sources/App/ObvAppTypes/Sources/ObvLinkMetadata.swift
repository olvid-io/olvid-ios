...
public final class ObvLinkMetadata: NSObject, ObvFailableEncodable, NSSecureCoding, @unchecked Sendable {
    ...
    private static var log: OSLog {
        return OSLog(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: self))
    }
    
    
    public init(title: String?, desc: String?, url: URL?, pngData: Data?) {
        self.title = title
        self.desc = desc
        self.url = url
    ...
    static public func decode(_ obvEncoded: ObvEncoded, fallbackURL: URL?) -> ObvLinkMetadata? {
