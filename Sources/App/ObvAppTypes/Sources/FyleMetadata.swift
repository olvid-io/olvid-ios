...
public struct FyleMetadata: Codable {
    ...
    private let logger = Logger(subsystem: ObvAppCoreConstants.logSubsystem, category: String(describing: FyleMetadata.self))
    ...
    public init(fileName: String, sha256: Data, contentType: UTType) {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let _mimeType = contentType.preferredMIMEType {
            ...
            let contentTypeDebugDescription = self.contentType.debugDescription
            logger.error("Could not find appropriate MIME type for content type \(contentTypeDebugDescription, privacy: .public). We fallback on Data")
