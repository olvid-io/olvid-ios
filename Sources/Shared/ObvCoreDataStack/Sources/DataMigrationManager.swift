...
open class DataMigrationManager<PersistentContainerType: NSPersistentContainer> {
    ...
    public init(modelName: String, storeName: String, transactionAuthor: String, enableMigrations: Bool, migrationRunningLog: RunningLogError) {
        ...
        self.logger = Logger(subsystem: "io.olvid.messenger", category: logCategory)
    }
    ...
    public func deleteStore() throws {
        if storeExists() {
            self.logger.warning("Will delete Data Store")
            do {
                try FileManager.default.removeItem(at: storeURL)
            } catch {
                self.logger.fault("Failed to delete Data Store: \(error.localizedDescription)")
                throw error
            }
            self.logger.warning("Did delete Data Store")
        }
    }
