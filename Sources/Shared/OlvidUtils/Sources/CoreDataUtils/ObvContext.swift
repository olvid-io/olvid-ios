...
public final class ObvContext: Hashable, CustomDebugStringConvertible {
    ...
    /// Wrapper trick to avoid "sendable" issues in the `NSManagedObjectContextDidSave` handler.
    /// Note that the value cannot be set and read at the same time, hence the `@unchecked Sendable`.
    private final class SaveWasCalledWrapper: @unchecked Sendable {
    ...
    public init(context: NSManagedObjectContext, flowId: FlowIdentifier, file: StaticString, line: Int, function: StaticString) {
        ...
        self.flowId = flowId
        self.token = NotificationCenter.default.addObserver(forName: Notification.Name.NSManagedObjectContextDidSave, object: context, queue: nil) { [saveWasCalled] (notification) in
            if !saveWasCalled.value {
...
public protocol ObvManagedObject: NSManagedObject {
    var obvContext: ObvContext? { get set } // **MUST** be declared as "weak"
}
