...
public final class CompositionOfFourContextualOperations<ReasonForCancelType1: LocalizedErrorWithLogType,
                                                         ReasonForCancelType2: LocalizedErrorWithLogType,
                                                         ReasonForCancelType3: LocalizedErrorWithLogType,
                                                         ReasonForCancelType4: LocalizedErrorWithLogType>: OperationWithSpecificReasonForCancel<CompositionOfFourContextualOperationsReasonForCancel<ReasonForCancelType1,
                                                                                               ReasonForCancelType2,
                                                                                               ReasonForCancelType3,
                                                                                               ReasonForCancelType4>>, @unchecked Sendable {
    ...
    public override var debugDescription: String {
        let concatanatedOpsNames = [op1, op2, op3, op4].map({ $0.debugDescription }).joined(separator: "->")
        let thisOperationName = "CompositionOfFourContextualOperations"
        return "\(thisOperationName)[\(concatanatedOpsNames)]"
    }
    
    
    /// See ``CompositionOfTwoContextualOperations.cancel(withReason:)``.
    public override func cancel(withReason reason: CompositionOfFourContextualOperationsReasonForCancel<ReasonForCancelType1, ReasonForCancelType2, ReasonForCancelType3, ReasonForCancelType4>) {
        ...
        obvContext.performAndWait {
            do {
                guard obvContext.context.hasChanges else {
                    debugPrint("🙃 No need to save completion handler for op1: \(op1.debugDescription), op2: \(op2.debugDescription), op3: \(op3.debugDescription), op4: \(op4.debugDescription)")
                    return
                }
    ...
    private func logExecutionDurationToDisplayableLog() {
        ...
        ObvDisplayableLogs.shared.log("[⏱️][\(duration)] [CompositionOfFourContextualOperations<\(op1.description)->\(op2.description)->\(op3.description)->\(op4.description)>]")
    }

}


public enum CompositionOfFourContextualOperationsReasonForCancel<ReasonForCancelType1: LocalizedErrorWithLogType,
                                                                  ReasonForCancelType2: LocalizedErrorWithLogType,
                                                                  ReasonForCancelType3: LocalizedErrorWithLogType,
                                                                  ReasonForCancelType4: LocalizedErrorWithLogType>: LocalizedErrorWithLogType {
    ...
    public var logType: OSLogType {
        switch self {
        case .unknownReason, .coreDataError, .op1HasUnfinishedDependency, .op2HasUnfinishedDependency, .op3HasUnfinishedDependency, .op4HasUnfinishedDependency:
            return .fault
